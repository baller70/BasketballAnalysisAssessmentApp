...
# ShotIQ.xcodeproj is GENERATED by XcodeGen from project.yml and is not in git,
# so on a fresh clone it does not exist yet and every xcodebuild call below
# fails with "does not exist" — a confusing way to say "run xcodegen". The
# CI workflow does `brew install xcodegen && xcodegen generate` before it
# builds; a device install needs exactly the same step.
if [ ! -d "$project" ]; then
  [ -f project.yml ] || die "neither ${project} nor project.yml is in ${PWD}"
  ...
  if [ -z "$xcodegen_bin" ]; then
    ...
    [ -n "$brew_bin" ] || die 'xcodegen is not installed and neither is Homebrew — install XcodeGen first'
    note 'XcodeGen is missing; installing it with Homebrew.'
    "$brew_bin" install xcodegen || die 'could not install xcodegen'
    xcodegen_bin="$(command -v xcodegen || echo /opt/homebrew/bin/xcodegen)"
  fi

  "$xcodegen_bin" generate || die 'xcodegen could not generate the project'
  [ -d "$project" ] || die "xcodegen ran but ${project} still is not there"
