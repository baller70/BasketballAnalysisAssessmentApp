...
# Verbatim from baller70/hooptrack (branch claude/hooptrack-xcode-app-store-hnn1vt),
# where it powers the working release lane — same Apple account, same team,
# same machine, so ShotIQ reuses the same key.
#
...
#
# and reads those three out of its worker/worker.env.
#
