...
for (const route of ROUTES) {
  ...
  for (let i = 0; i < limit; i++) {
    try {
      ...
      const after = await page.evaluate(() => ({ url: location.pathname, html: document.body.innerHTML.length })).catch(() => before)
      if (!(changed || after.url !== before.url || after.html !== before.html)) row.deadButtons.push(label)
