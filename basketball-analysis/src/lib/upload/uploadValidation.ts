...
export function validateFileFormat(file: File): { valid: boolean; message: string } {
  const isVideo = UPLOAD_CONSTANTS.VIDEO_FORMATS.includes(file.type)
  const isImage = UPLOAD_CONSTANTS.IMAGE_FORMATS.includes(file.type)
...
export function getUploadType(file: File): UploadType {
  return UPLOAD_CONSTANTS.VIDEO_FORMATS.includes(file.type) ? "video" : "image"
}
...
export function isValidFileType(file: File): boolean {
  return UPLOAD_CONSTANTS.VIDEO_FORMATS.includes(file.type) || 
         UPLOAD_CONSTANTS.IMAGE_FORMATS.includes(file.type)
}
