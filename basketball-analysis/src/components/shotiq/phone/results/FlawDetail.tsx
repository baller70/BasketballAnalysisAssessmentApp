...
export function FlawDetail({
  ...
  return (
    ...
    >
      <ResultsIdentity className="mt-[10px] px-[21px]" name={name} streak={streak} points={points} />
      <span aria-hidden="true" className="mt-[9px] block h-px" style={{ background: RULE }} />

      {/* title + score --------------------------------------------------- */}
      <div className="mt-[10px] flex items-start px-[21px]">
        <div className="min-w-0 flex-1">
          <div className="shotiq-section-label text-[12px] leading-[12px] tracking-[0.08em]">FLAW DETAIL</div>
          <h1 className="shotiq-display mt-[3px] text-[27px] leading-[27px] tracking-[0.02em]">{title}</h1>
          <p className="mt-[7px] text-[13px] leading-[16px]">{desc}</p>
        </div>
        <div className="ml-[12px] w-[104px] shrink-0 text-right">
          ...
          <div className="shotiq-numeric text-[42px] leading-[40px]" style={{ color: ORANGE }}>{score}</div>
          <span className="ml-auto block w-[104px]"><ScoreBar score={score} width={104} height={5.5} /></span>
        </div>
      </div>

      {/* meta chips ------------------------------------------------------- */}
      <div className="mt-[10px] flex divide-x divide-[var(--shotiq-color-rule)] px-[21px]">
      ...
      <div className="mt-[13px] px-[19px]">
        ...
        <div className="mt-[5px] flex gap-[1px] overflow-hidden rounded-[3px]">
          {PHASE_STILLS.map((s, i) => (
            <span key={s} className="relative block min-w-0 flex-1" style={{ height: 134 }}>
      ...
      {/* impact --------------------------------------------------------------- */}
      <div className="mt-[12px] flex gap-[12px] px-[21px]">
        <div className="w-[168px] shrink-0">
          <div className="shotiq-display text-[20px] leading-[20px] tracking-[0.04em]">IMPACT</div>
          <p className="mt-[5px] text-[12.5px] leading-[15px]">
          ...
          {([["YOUR ANGLE", yourAngle, ORANGE, "rgba(253,55,1,.12)"], ["IDEAL RANGE", idealRange, BLUE, "rgba(45,108,223,.12)"]] as [string, string, string, string][]).map(([l, v, c, fill]) => (
            <span key={l} className="min-w-0 flex-1 px-[6px] text-center">
              ...
              <span className="mt-[5px] flex items-center justify-center gap-[4px]">
                <PoseGlyph phase="release" size={46} accent={c} />
                <span className="relative block h-[46px] w-[36px]">
      ...
      <div className="mt-[12px] flex gap-[12px] px-[21px]">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-[8px] rounded-[6px] px-[10px] py-[8px]" style={{ background: "#EDF3FD" }}>
          <span className="min-w-0">
            ...
            <span className="mt-[5px] block space-y-[4px]">
              {["Elbow under ball", "Forearm vertical", "Wrist behind ball"].map((t) => (
                <span key={t} className="flex items-center gap-[6px] text-[12px] leading-[14px]">
          ...
          <span className="ml-auto shrink-0"><CorrectionGlyph kind="stack" size={50} /></span>
        </div>
      </div>

      {/* recommended drill ------------------------------------------------------ */}
      <div className="mt-[11px] px-[21px]">
        <div className="shotiq-display text-[19px] leading-[19px] tracking-[0.05em]" style={{ color: GRAPHITE }}>RECOMMENDED DRILL</div>
        <Panel className="mt-[5px] flex items-center gap-[11px] p-[8px]">
          <Frame src="090-lib-1" w={78} h={60} radius={4} alt="Towel Elbow Stack drill" />
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-[3px] block text-[12px] leading-[14.5px]">
              Use a towel between elbow and hip to build awareness of keeping your elbow stacked through release.
            </span>
          </span>
          <CorrectionGlyph kind="drive" size={44} />
          <Chev size={15} />
        </Panel>
      </div>

      {/* actions ------------------------------------------------------------------ */}
      <div className="mt-[10px] flex gap-[10px] px-[21px]">
        <Panel className="flex h-[30px] flex-1 items-center justify-center gap-[8px]">
