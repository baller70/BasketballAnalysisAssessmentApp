...
102. **A REFUTING MEASUREMENT TAKEN AFTER THE BAND IS AN ATTRIBUTION CHECK, NOT
    A REFUTATION, AND IT MUST BE RECORDED UNDER THE WEAKER NAME.** Rule 101 asks
    a grader to supply a landmark measurement the builder can take BEFORE
    scoring, so that a band movement is attributable rather than merely
    welcome. Round 39 took all three of them AFTER reading the bands, and the
    difference is not pedantic: my own estimator came back disagreeing with the
    grade on one of the three, and because I already knew the band had landed
    the prediction to four decimals, I went looking for the fault in my
    instrument — and found it there. That was the correct answer, and I reached
    it from the wrong direction. Had the refutation come first, the disagreement
    would have been a genuine fork with real cost, which is exactly the cost
    rule 101 is buying.

    So: take the landmark measurement before the capture is scored, or label the
    result an ATTRIBUTION CHECK when writing it down. Both are worth having. Only
    one of them can refute anything.

    The corollary is about instruments, and it cuts the other way from most of
    this file: **an estimator you wrote yourself is not automatically the more
    trustworthy one.** Mine merged adjacent glyphs at a gap threshold the grade's
    did not, returning n=9 where it returned n=23, and a merged cluster's
    registration is dominated by its widest member. When two estimators of the
    same quantity disagree, add a THIRD that shares no parameter with either —
    here, a single sub-pixel offset fitted over the whole ink profile, which has
    no segmentation to get wrong — rather than adjudicating between the two.

103. **TWO ESTIMATORS AGREEING IS NOT TWO INDEPENDENT ESTIMATORS — THEY CAN
    SHARE A BLIND SPOT, AND THE ONE THAT MATTERS IS USUALLY GEOMETRIC.** Rule
    102 says that when two estimators disagree you add a third. The converse trap
    is worse and cost this screen three refusals to find: on `shield` and `gear`,
    the ink CENTROID and a whole-profile SSD offset BOTH said "translate up", and
    BOTH were wrong. A height deficit anchored at one end displaces a centroid
    exactly as a translation does, and an SSD fit over a profile that is the
    wrong height finds its best compromise by sliding. Neither estimator can see
    the difference, so their agreement carries no information about it.

    **PAIRED EDGE CROSSINGS CAN, because they measure the two ends separately.**
    Edges moving in the SAME direction is a translation; edges moving OPPOSITELY
    is a size error; one edge landed and one edge off is a size error plus a
    displacement, and it decomposes. shield read top -0.210/-0.114 against bottom
    +0.462/+0.362; gear top +1.332/+1.460 against bottom -0.142/-0.346. Both are
    sizes, and every centroid-driven prescription against them was paper-over.

    So when choosing a third estimator, do not reach for another integral of the
    same ink — reach for one that resolves the thing the first two average away.
    And prefer a CROSSING to an envelope (rule 90): a crossing is an interpolated
    level, an envelope is an extreme-value pixel and canonical's unsharp ramp
    runs further on both sides.

    Round 40's D1 is the payoff. The shield had ONE edge landed and ONE edge 0.79
    px short — which is not a displacement, and is why every earlier attempt to
    move that mark was correctly refused. Widening the box closed it, and the
    refutation that licensed it was the LEFT edge holding at -0.075 instead of
    running to +0.25.

104. **NEVER SWEEP ONE HALF OF A PAIR THE EMITTER COMPENSATES — GRID BOTH, AT A
    STEP FINER THAN THE RASTER PLATEAU.** `runCss` emits `scaleX(s) scaleY(sy)
    translate(tx,ty)` at `transform-origin: 0 0`, which maps local y to
    `sy*(y+ty)`. So `sy` does not only resize a run: **it moves the baseline**,
    and `ty` is multiplied by it. The two are a compensated pair by construction,
    and a 1-D sweep of `sy` at fixed `ty` measures "resize AND displace", which
    is almost always worse than either alone.

    That is not hypothetical. A bare `sy` sweep called `display`, `lede1`,
    `resendVal`, `resendLink`, `safe1` and `plateLab` argmin — **and every one of
    them gains once `ty` is allowed to move with it.** Round 41's two findings
    were both inside that set. Any earlier "argmin on `sy`" in this file measured
    nothing and should be treated as unmeasured.

    The step matters as much as the dimensionality. Vertical position is
    quantised to ~1.0 device px (rule 100), i.e. ~0.46 CSS px here, so a `ty` grid
    coarser than that can step straight over the plateau that contains the
    optimum. Grade 26 used 0.20 CSS px, which samples every reachable vertical
    position at every `sy`.

    The general form: before sweeping a parameter, read the emitter and ask what
    ELSE that parameter moves. `sy`/`ty` is the instance this screen found; `skew`
    and `scaleX` were another (round 4, where two rounds of advance solving had
    silently flattened a slant, because CSS multiplies a `skewX` by the `scaleX`
    that follows it). Same lesson, five rules apart: **the transform list is not a
    set of independent knobs.**

105. **VALIDATE THE INSTRUMENT IN THE EDIT FAMILY YOU ARE ABOUT TO PRESCRIBE IN,
    NOT MERELY ON THE SCREEN — AND THE STRONGEST VALIDATION IS A NUMBER THIS FILE
    ALREADY RECORDED.** Rule 40's control proves a rig reproduces the SHIPPED
    raster. That is necessary and it is not sufficient: it says nothing about
    whether the rig reproduces a CHANGE, which is the thing a prescription is.

    Grade 26 rebuilt round 33's exact `lede1` prescription and got back the band
    this file recorded seven rounds earlier. Grade 27 did the same one emitter
    over, because its target was `MARK_BOXES` and not `RUNS`: it reverted round
    40's `diffMark` edit on top of HEAD and recovered **both** bands, 9.0344 and
    3.8481, to four decimals. Then it went further and calibrated its ESTIMATOR on
    that same build — geometry predicted the edit would change its `stretch`
    statistic by +0.270 and the measurement returned +0.270 exact.

    That is three separate things proved before a single new number was
    prescribed: the rig reproduces the present, it reproduces a recorded change in
    the right emitter, and the estimator responds to a known geometric quantity by
    the right amount. A rig that passes all three has earned a prediction to four
    decimals, and both graders that did it got one.

    The corollary for THIS file: every round entry that records a band movement is
    a regression test for some future instrument. Write the before AND after
    numbers, per band, always — they are not decoration, they are the only
    historical ground truth an instrument can be checked against.

106. **A CROSSING IS NOT COMPARABLE ACROSS THE TWO RASTERS — NOT AT A FRACTION
    OF PEAK, AND (AMENDED BY GRADE 32) NOT AT AN ABSOLUTE LEVEL EITHER WHEN THE
    TWO EDGES HAVE DIFFERENT SOFTNESS. USE AN INTEGRAL WITH A LINEARITY
    ARGUMENT.**

    The amendment cost a whole diagnosis. Paired absolute-level 50% crossings said
    `display`'s ink sits -1.12 left and -2.08 right of canonical, consistently
    over 118 scan lines — which rule 103 reads as a translation. It is an
    artefact: canonical climbs 371/843/6837/19602/29256 over FIVE columns where
    the render climbs 291/8765/21169/29681 over THREE, so **any threshold sits
    about a pixel further right in the softer raster.** The "correction" costs
    +1.69 of band. When two edges differ in softness, no level — fixed or
    fractional — measures the same thing on both. Canonical
    carries an unsharp mask. That does three things: it OVERSHOOTS the core (a
    chevron arm measured **coverage 1.194**, i.e. past saturation), it sharpens
    the flanks, and it is antisymmetric on a step. The third property is the
    useful one: a crossing taken at an ABSOLUTE level is preserved by the mask,
    while a crossing taken at a fraction of the LOCAL PEAK is not, because the
    peak itself moved.

    On the chevron stroke that difference is not subtle. A 50%-of-peak crossing
    reported canonical's arm **19% thinner** than the render's; three levels of
    the same estimator gave **19% / 13% / 6%**. The true figure is **2.7%**. When
    an estimator's spread across its own free parameter exceeds the quantity it
    is measuring, it is broken, not noisy — and averaging its readings hides
    exactly that.

    Two replacements, both used successfully here:
      * a **fixed coverage level** (0.5 of the window's paper/ink pair, NOT of the
        local peak), which is what round 43's ring and hands landmarks use; and
      * an **integral with a linearity argument** — the row-cut integral divided
        by `sqrt(1+m^2)` for a stroke of slope `m`. An unsharp mask is linear and
        zero-sum, so it conserves a row-cut integral taken wide enough to contain
        the halo; resampling conserves it too. That makes it unbiased ACROSS the
        two rasters, which is the only property that matters when the whole task
        is comparing them.

    Validate whichever you choose against a GEOMETRIC prediction and for
    LINEARITY before prescribing (rule 105 part 3). Round 43's integral predicted
    2.5972 from the emitter and read 2.5896, and returned slope 1.00 over twelve
    builds — so its 2.531 target could be trusted to convert one-for-one into an
    `sw`.

107. **A LANDMARK MEASURED AGAINST A DEFECT THAT A SIBLING EDIT IS ABOUT TO FIX
    WILL LIE TO YOU — RE-TAKE EVERY LANDMARK ON THE COMPOSITE.** Round 43 nearly
    shipped a fifth edit. Measured on its own, the clock hands' ROW read -0.386
    and a correction looked justified. Measured on the composite — after the
    dial's `cy` had been corrected — the same row reads **+0.043, already
    landed**, and the "correction" would have driven it to +0.398.

    The mechanism is general and it is not the same as rule 91's masking. There, a
    defect hides UNDER another defect. Here, a landmark is contaminated BY another
    defect, so the reading is real, the diagnosis is wrong, and the objective
    agrees with the wrong diagnosis (-0.0004) because the sibling fix has not been
    applied yet.

    So the order is: build the composite FIRST, then re-take every landmark on it,
    and only then decide which edits survive. An edit that was justified against
    the shipped raster and is not justified against the composite is not a
    marginal call — it is a defect that no longer exists.

    **AND THE RULE IS SYMMETRIC, which round 44 found and this text originally
    missed.** It can LICENSE an edit as well as kill one. Round 43 refused `ry`
    for a correct reason: the dial's two vertical edges then read +0.247 and
    +0.303, both positive, a translation, and `ry` would have driven the bottom
    to +0.56. Correcting the sibling `cy` removed the translation, and what
    remained was a size — +0.063 against −0.173, opposite signs — so `ry` became
    right. **An edit refused in an earlier round is not closed; it is closed
    against the raster that existed then**, and every sibling correction since is
    a reason to re-take its landmark.

108. **A WRONG DERIVATION CAN HAVE A RIGHT VALUE. FINISHING AN AUDIT "BY
    SYMMETRY" IS NOT A MEASUREMENT, AND HERE IT WOULD HAVE COST MORE THAN THE
    ROUND BEFORE IT WON.** Rounds 41-43 found three constants derived from a
    wrong input, and the natural conclusion — sweep the file and correct every
    other constant with the same provenance — is wrong.

    `MailClockMark`'s `cx = 17.5 + 2.77/k` uses the k this file proves wrong.
    Correcting it consistently gives 18.5027, a 0.655 px move. It is **at
    argmin at 18.74**: `18.5027` scores **+0.0051 / n_over8 +20** and takes the
    band to worse than it was two rounds earlier, while 18.68 and 18.80 straddle
    it. The reason is that its INPUT was also wrong — 2.77 px from a circle fit
    taken when the ring was 20% oversized and merged with the envelope — by
    almost exactly the compensating amount. Two errors, one value, correct.

    So the audit's output is a MEASUREMENT per constant, never a rewrite per
    provenance. The refuting measurement is usually cheap and always comes first:
    a 0.655 px `cx` error must move both horizontal outer edges the same way by
    0.655, and the measured translation was **-0.052**. That one number settled it
    before anything was built.

    The general form, and it is the mirror of rule 91: a defect can hide under a
    defect, and **a correct value can hide under a wrong derivation**. Both are
    invisible to the reasoning and visible to the measurement. Fix the COMMENT so
    the next round is not misled, and leave the number alone.

    Corollary on prose: a comment that states a measurement is an input to the
    next round's arithmetic. This file has now been wrong four times because a
    stale number in a comment was believed — `lede2`'s ascender, `helpIcon3`'s
    height, the clock hands' offset, and the two assertions round 44 corrected.
    **When a round moves a value, fix every sentence that quotes it in the same
    commit.**

109. **A DEFECT CAN BE MEASURED EXACTLY, CORRECTED EXACTLY, LAND EVERY
    LANDMARK IT PREDICTED, AND STILL BE REFUSED BY THE OBJECTIVE. RECORD IT WITH
    ITS NUMBERS AND ITS COST — DO NOT SHIP IT AND DO NOT DROP IT.** This is a new
    category and it is distinct from every refusal above it. A paper-over is a
    gain that moves a landmark the wrong way. An unreachable residual is a defect
    no lever touches. **This is a defect a lever reaches perfectly, whose
    correction the metric declines.**

    Round 45's chevron aspect is the case. Two statistics sharing no parameter
    agreed to 0.05 percentage points (5.09% by arm slope, 5.04% by paired
    tip/vertex extrapolation); the emitter makes the drawn slope *identically*
    `bw/bh`, so the aspect is the parameter and not an approximation; the
    correction landed slope 0.9378 -> 0.8933 against a prediction of 0.8925 and a
    canonical 0.8956, took tip and vertex from -0.252 / +0.251 to +0.031 / +0.007,
    and closed the arm width to 2.5139 against canonical's 2.5099 as a *free
    consequence* — one cause explaining both symptoms. **And the whole-screen
    figure went +0.0003.** The metric's own argmin on that axis leaves the drawing
    most of the way wrong.

    WHY IT HAPPENS: `mean|d|` over a window rewards ink OVERLAP, and a small
    rotation of a thin diagonal loses overlap on both arms while gaining nothing
    a pixel grid can see. The geometry improves and the proxy does not follow.

    WHAT TO DO. Write it into the recipe next to the value, with the measurement,
    the exact correction, and the built cost — the same footing as a stated
    unreachable residual, and for the same reason: **the next round must not
    re-find it as free money, and must not "correct" it either.** Do not ship it,
    because rule 71 and the whole-screen figure are the contract this file has
    with itself.

    AND THE LARGER READING, which matters for the DEFINITION OF DONE: `mean|d|`
    is a PROXY for "matches canonical", and this is the first time the two have
    been shown to disagree on a quantity measured to better than 1%. When a screen
    starts producing findings in this category, the metric has stopped being the
    binding constraint and the remaining error is a question for the eye and for
    Kevin, not for the objective. Round 45 produced one such finding and one
    ordinary one. **If a round produces only findings of this kind, that is the
    floor, and it should be recorded as such rather than ground against.**

110. **THE NEXT FINDING IS USUALLY ON AN AXIS THE RECIPE DECLARES AND NO ROUND
    HAS SWEPT — SO ENUMERATE WHAT THE FILE DECLARES, NOT WHAT PREVIOUS ROUNDS
    MEASURED.** Three times in seven rounds the largest available finding was
    sitting on a parameter that was fully implemented, fully emitted, and never
    tested:

        grade 25   `letter-spacing`   declared and emitted; unswept for 40 rounds
        grade 26   `sy` as a 2-D grid  swept only as a bare 1-D sweep, which
                                       measures nothing (rule 104)
        grade 31   the colour tokens   ~2,000 built candidates of geometry, and
                                       no colour audit at all

    The failure mode is not laziness, it is INHERITANCE. Each round reads the
    previous round's brief, which lists what is open by BAND, and bands are
    geometry-shaped. Nothing in that loop ever asks "what can this recipe
    actually change?" Grade 31's finding was one character in a token whose
    correct value was written in the file's own docstring.

    The mechanism that hid it is worth naming separately: **rule 71 closed the
    LUMINANCE axis and was read ever after as closing COLOUR.** A refusal names
    the axis it was measured on. Chromaticity is not luminance, and no round had
    tested it. When citing an old refusal, re-read what it actually measured.

    THE PRACTICE: before choosing a target, list every parameter the recipe
    emits — every field of `Run`, every entry of `MARK_BOXES`, every CSS custom
    property, every literal in a path — and mark which have been swept AT ALL.
    The unswept column is short and it is where the findings are. That list is a
    property of the SOURCE and takes ten minutes to build; the band table is a
    property of the last round's arithmetic and will not produce it.

111. **A GRID SEARCH'S BEST CELL IS NOT A FINDING UNLESS IT BEATS ITS OWN
    ORDER STATISTIC AND ITS NEIGHBOURS BRACKET IT.** Search twenty cells on a
    noisy surface and the minimum of twenty draws sits about 1.87 standard
    deviations below the mean **by construction, with no signal present at all.**
    Report that minimum as a gain and you are fitting the raster lattice.

    Grade 32's best cell — `display tx 4.52 / skew -10.2` — scored whole
    **-0.0133** and n_over8 **-83**, the largest number found in six rounds, and
    it was declined on the statistics of its own search:

        cell sd 0.1063, range 0.397 over 20 cells   -> a scatter, not a surface
        best cell 1.98 sd below the mean            -> E[min] of 20 draws is 1.87
        one-step neighbours span 0.139              -> against a claimed gain of
                                                       0.168: NOT BRACKETED

    THE THREE TESTS, and a candidate must pass all three:
      (a) **Bracketing.** The neighbours on both sides must be worse by more than
          the local scatter. A cell whose neighbours straddle it is a lottery
          ticket, however good its number.
      (b) **The order statistic.** Compare the best cell's z-score to E[min] for
          the number of cells searched, not to zero. 1.98 against 1.87 is nothing.
      (c) **A signal independent of the objective.** Grade 32 found one and
          reported it honestly at its true size: `tx 4.52` beat `4.60` at all four
          skews, sign test p=0.125, a residual of ~-0.10 device px — inside the
          noise, and stated as such rather than inflated into the -0.0133.

    This is the counterpart to rules 34 and 71, which refuse a gain because a
    LANDMARK says it is wrong. Rule 111 refuses a gain because the SEARCH says it
    is not there. Both are ways the objective lies, and on this screen the second
    only became visible once the per-round gains fell to the size of Skia's
    sub-pixel quantum — which is itself a signal that a screen is at its floor.
...
97. **A GEOMETRIC CORRECTION HAS A FIXED RE-RASTERISATION COST, SO BELOW SOME
    ...
    sweep. Five paid, all improving the mean AND n_over8. The sixth went backwards
    on both. It was not the least confident — both runs the grade flagged MEDIUM
    beat their predictions — it was the SMALLEST: caps 2.4% to 14% out all paid,
    and the one at 1.89% did not.

    The mechanism is that an image-space sweep RESAMPLES the existing raster while
    the build RE-RASTERISES the glyphs at a new size. Re-rasterisation moves stems
    and hinting by an amount that does not shrink with the correction, so the
    budget is `gain(magnitude) - cost(roughly constant)` and there is a
    break-even. On this screen, for type, it sits near 2.4% — lede1 failed at
    1.89%, resendVal at 2.3%, and five runs at 2.4% to 14% all paid.

    **BOUNDED BY ROUND 36: THIS IS A STATEMENT ABOUT SIZE CHANGES, NOT ABOUT
    SMALL CHANGES.** A `word-spacing` correction of 0.671% — a third of the
    smallest size change that has ever failed here — BEAT its prediction, because
    `word-spacing` is a LAYOUT property: it moves glyphs without re-rasterising
    them at a new em, so the fixed hinting-and-stem cost simply does not arise.
    Before applying the cutoff, ask whether the lever changes WHAT is rasterised
    or only WHERE it sits. Translations through `transform` and layout-space
    spacing are free of it; `size`, `scale` and `sy` are not.

    Consequences. Sort a batch of same-kind prescriptions by MAGNITUDE and expect
    a cutoff, rather than expecting them all to hold or all to fail. When one of a
    batch is refuted, check whether it is the smallest before concluding anything
    about its diagnosis — lede1's cap measurement was not wrong, it was merely too
    small to be worth acting on. And a sweep's predicted gain is an UPPER BOUND
    for a real build whenever the build changes what is rasterised rather than
    where it is placed; a pure translation through `transform` carries no such
    cost, which is why `tx`/`ty` predictions on this screen land to four decimals
    and size predictions do not.

98. **A TRAP THAT HAS FIRED THREE TIMES IS A MISSING GUARD, NOT A LAPSE.**
    `PHONE_CSS` is a template literal, so a backtick anywhere inside it ends the
    string. Writing CSS comments in this file's own house style — backticks around
    identifiers — has broken the build three separate times, twice with TS1005 and
    once with TS1443. Each time it was found in seconds by running tsc, fixed, and
    not prevented.

    The general form: when the same mistake recurs, stop attributing it to
    attention. Attention does not scale and it does not persist across a context
    boundary. Either the affordance changes — write the comment ABOVE the literal
    rather than inside it, or use quotes inside it, and say so where the literal
    begins — or a check catches it. Here both are cheap, and the check already
    exists: tsc as its own step, before the build, which is why the cost has
    always been a minute rather than a round.

    The corollary is about what to write down. A recurring trap belongs in the
...
100. **THE VERTICAL TEXT POSITION IS QUANTISED TO 1.0 DEVICE PX, SO MOST
    SUB-PIXEL `ty` PRESCRIPTIONS ARE UNREACHABLE.** Two of round 35's three
    translations produced byte-identical rasters while the served CSS carried the
    new value. A live sweep with the injection read back gives three distinct
    rasters across +/-1.2 device px, with a ~0.9 px plateau containing the shipped
    position. A run's reachable set is therefore {shipped, shipped +/- 1, ...},
    and any argmin inside the plateau is a fiction of the estimator.

    So: before building a vertical text move, SWEEP THE PLATEAU and find the
    nearest boundary. If the measured error is smaller than the distance to it,
    the correct action is to state the residual, not to move. If it is larger, the
    move is a whole pixel and the prediction should be computed at a whole pixel.

    Two corollaries. An image-space objective will happily return a fractional
    argmin that no build can express — it resamples, the emitter quantises — so a
    fractional `dy` from a sweep is a MEASUREMENT of the error, never a
    prescription. And this is why every `ty` prediction on this screen that landed
    to four decimals was a full pixel or more: the exceptions were not luck, they
    were the only reachable moves.

    Horizontal is different and must not be assumed to match — `tx` changes of
    0.2-0.5 device px have moved bands on this screen repeatedly. Sub-pixel
    positioning survives in x and dies in y.

101. **VERIFYING THAT A LEVER IS WIRED IS NOT VERIFYING WHAT IT DOES.** D4's
    premise was checked twice before it was built — the `vector-effect` injection
    read back as applied in `getComputedStyle`, and the viewBox arithmetic was
    read off the component rather than assumed. Both were right. The realised
    stroke still came out at `sw x sx` instead of `sw`, because this Chromium
    resolves `non-scaling-stroke` against a coordinate system that keeps the
    horizontal scale.

    Rule 99 says prove the injection took. That is necessary and it is not
    sufficient: "the declaration applied" and "the declaration did what the spec
    says" are two claims, and only the second is the one a prescription rests on.
    So the last assertion before a band is read must be a measurement of the
    EFFECT in the units the prescription is written in — here, one wall's width in
    device px.

    Grade 22 wrote that assertion into its own prescription, with the number and
    the tolerance, and it is the reason this cost one build instead of a round of
    misattributed band movements. **A prescription that cannot name the
    measurement that would refute it is not finished.** Ask for it.

    **THE EXAMPLE ABOVE IS WRONG, AND THE CORRECTION SHARPENS THE RULE.** Grade 23
    showed that round 37 did not measure a mis-implemented `vector-effect` — it
    measured NO EXPERIMENT. `vector-effect` is not inherited, `Icon` carries
    `strokeWidth` on the `<g>`, the build put `vectorEffect` there too, and the
    paths kept `vector-effect: none`. The realised 8.879 is exactly what removing
    the geometric mean and adding nothing predicts (8.788), and the property on
    the PATH lands isotropically at `sw x S` — the constant the grade guessed.
    So rule 99's "prove the injection took" was satisfied on a PROBE and the BUILD
    wrote the declaration where it could not apply. **Proving a declaration
    applied to element X is not proving it applied to the element that paints** —
    read back the computed style on the SHIPPED element, not on a stand-in.
...
### GRADE 32 (**A−**), AND NO ROUND 47: THE AXIS INVENTORY IS COMPLETE, EVERY CANDIDATE IS REFUSED BY ITS OWN LANDMARKS, AND **THIS IS THE FLOOR — WITH THE EVIDENCE THE LAST FLOOR CLAIM LACKED**

**Nothing was prescribed and nothing was built. The figure stands at 4.1139 /
n_over8 90508.** That is the result, and it is the first time in thirty-two
grades that it is the right one.

**WHY THIS FLOOR CLAIM IS DIFFERENT FROM THE ONE ROUND 46 REFUTED.** The earlier
claim rested on a collapsing gain series, and it was wrong because every one of
the ~2,000 candidates behind it swept GEOMETRY while the band table quietly
framed each brief the same way. The standing requirement after that was an
**inventory of what the recipe DECLARES** (rule 110). Grade 32 built it, from the
file rather than from the band table, tracing all 21 `Run` fields through
`runCss`:

    stroke   0 uses    NOT DEAD — a live lever. Its use is rule-109 class; see below.
    skew     1 use     NULL, and measured rather than assumed. See below.
    dx / dy  21 uses   NOT AXES. `runCss` emits only `top-dy-oy` and `x-dx-ox`.
                       Built display `top 228.686 -> 222.286` with `dy 36.4 -> 30.0`:
                       BYTE-IDENTICAL. Pure reparametrisation.
    bang     1 use     DEAD in this instrument — removed, byte-identical. It can
                       only bite against a competing declaration and the
                       raster-exact instrument carries none. KEEP AS A GUARD.
    width    9 uses    Live but DEGENERATE with `tx` — it moves the run as a
                       translation, and the shipped value is a sharp optimum
                       (348.3 -> 340 costs +0.1798; -> 360 costs +0.2509).
    ox / oy  6 uses    exact-cancel corrections; the DIFFBTN null is already recorded.
    family   7 uses    face, closed with the four-cut measurement.
    x cx top size weight scale sy ls ws tx ty     all swept.

    Semantic counts differ from a grep: `width` 9 not 22, `dx`/`dy` 21 not 22,
    `family` 7 not 9. The inventory has to be traced, not counted.

**AND ALL FIVE CSS CUSTOM PROPERTIES NOW HAVE A PLATEAU AUDIT.** Estimator: L1
argmin of canonical over the pixels the CONTROL paints at exactly the token,
eroded by k so nothing sits within k px of a ramp.

  * **paper** 254/254/254, gain 0.0000 at k=0,1,2 over 1.28M px. CLEAN.
  * **orange** 253/67/1, gain 0.0000 at k=0,1,2 — **round 46 confirmed on an
    erosion ladder round 46 did not run.**
  * **`--s5-ink` — CLEAN, and this is a NEW NULL that looked for a moment like a
    0.022 finding.** The k=2 argmin is 2/1/1 with gain 0.45/0.24/0.15 over 19,410
    px. It is a **one-sided bias, not a token error**: blur can only make an ink
    pixel LIGHTER, so on a contaminated core the median is biased up and the
    token is the FLOOR. The two histograms settle it — orange's plate core has
    mass on BOTH sides (65:7525, 66:21680, 67:21827, 68:9923), a true plateau,
    while ink's display core is a **spike at exactly 0 holding 36% of the mass**
    then monotone decay (0:4229, 1:1451, 2:1442, 3:1257). A shape only a token at
    0 can produce. Converged at k=2 and k=3. **Do not re-derive.**
  * **`--s5-box-rule` — admits no plateau audit at all**, and that is the honest
    verdict rather than a clean one. The stroke is 1.75 device px: 2,215 pure
    pixels at k=0 and **ZERO at k=1**. There is no interior anywhere on the
    canvas. Its k=0 argmin (115) is exactly the contaminated statistic the ink
    case just disqualified, and rule 104 blocks it independently (width and tone
    are the compensated pair this file already swept 7x7).

### THE NULLS ARE THE DELIVERABLE, AND TWO OF THEM COST REAL WORK

**`skew`: the staleness hypothesis was right about the arithmetic and wrong about
the value.** Rule 104 says the emitted c-term is `scale*tan(skew)`. Round 4
derived skew from c = 0.10655 **at scale 0.586**; the size solve then moved scale
to 0.5921 and nobody re-derived it — 0.10648 -> 0.10760, **+1.05% drift, the
exact bug round 4 itself recorded**. Measured by joint (shear, translation)
registration of the whole band: residual shear **+0.00020 px/px**, cleanly
bracketed, with a blur control showing the resampling kernel's symmetric dip does
not reach it. **The drift is real and it moved TOWARD canonical.** The 20-cell
grid then puts marginal means by skew at 19.3459 / 19.3493 / 19.3500 / 19.3340 —
**spread 0.0160 against a cell sd of 0.1063. The axis is flat.** `skew: -10.3`
is correct at HEAD.

**THE BEST-SCORING CELL OF THE ROUND IS A LATTICE LOTTERY AND WAS DECLINED AS
ONE.** `tx 4.52 / skew -10.2` scores whole **4.1006 (-0.0133)** and n8 **90425
(-83)** — the largest number anyone has found in six rounds. Grade 32 refused it
on the statistics of its own search: the 20-cell surface is a scatter (sd 0.1063,
range 0.397), the best cell sits **1.98 sd below the mean where E[min] of 20
draws is 1.87 sd**, and its one-step neighbours span 0.139 against a claimed gain
of 0.168 — **not bracketed**. This is Skia's sub-pixel quantum, which this file
already names and declines to fit. The one real signal is that tx 4.52 beats 4.60
at all four skews (sign test p=0.125), a residual of ~-0.10 device px, inside the
noise. **See rule 111 — that is a new class of refusal and it is the most
disciplined thing in the round.**

**RULE 106 KILLED A THIRD ESTIMATOR, AND ITS SCOPE IS WIDER THAN WRITTEN.**
Paired sub-pixel 50% crossings say display's ink sits -1.12 left and -2.08 right
of canonical over 118 scan lines — a translation by rule 103. It is an artefact:
canonical climbs 371/843/6837/19602/29256 over five columns where the render
climbs 291/8765/21169/29681 over three, so **a threshold sits ~1 px further right
in the softer raster**. A +1.0 translation costs +1.69 of band. Rule 106 is
amended: it applies to ABSOLUTE thresholds, not only to fractions of peak.

### THE ONE REAL FINDING IS RULE-109 CLASS, AND IT NAMES WHY `weight` COULD NEVER REACH IT

Canonical's four typed digits are set in a Tungsten cut this repository does not
contain, and **it is heavier on some glyphs and not others**. Baseline-corrected
ink mass, canonical/render: digit0 '2' **1.0126**, digit1 '8' **1.0655**, digit2
'4' **1.0662**, digit3 '7' **1.1593** — and the file's own independent number for
'7' agrees (*"22.744 device px of ink where Tungsten's outline gives 20.52, 10.9%
wider"*). Mass is the estimator because it is blur- and register-invariant.

**`weight` cannot reach it, and the reason is structural**: the fifteen Geist
runs are a VARIABLE font, so weight is already a swept continuous lever — but
**Tungsten is four discrete cuts**, so on the seven Tungsten runs
`-webkit-text-stroke-width` is *the only continuous ink lever on the screen*, and
it is used by nothing. That is why forty-six rounds of weight sweeps could not
find this.

Rule 91 decomposition through the shipping emitter:

        size 38.22 alone                  -0.0037   n8  -30
        size 38.30 alone (declined argmin) -0.0056   n8  -34
        size 38.22 + stroke 0.10          -0.0110   n8  -15
        size 38.22 + stroke 0.21          -0.0159   n8  -13     4x either knob

**REFUSED, on three separate grounds.** (a) **digit0's own landmark**: its ink
mass was ALREADY LANDED at 1.0126 and the uniform pair drives it to **0.9626**, a
5% overshoot the other way — flagged by the stem estimator BEFORE the build and
confirmed by it. A per-digit stroke fixes that and is barred by this file's own
constraint, *"a player who keeps typing must not see the digits change shape
halfway along the row."* (b) **The two halves disagree**: against size-alone the
stroke buys -0.0122 of mean and gives back 17-21 of n_over8 — round 31's graphite
shape. (c) Rule 107 re-take: size 38.30 alone is still correctly refused, digit3's
cap going 1.0053 -> ~0.993, further from 1.000.

**Recorded with exact values. Not prescribed.**

## 005 IS AT ITS FLOOR AND IS BLOCKED, NOT DONE

The distinction matters and it is the reason nothing is being marked complete.
**DoD item 9 requires an independent grader to score A or A+.** Thirty-two grades
have now been taken and the best is A−. What stands between this screen and an A
is not measurement and not effort — it is **five decisions no measurement can
make**, every one of them now proved by build rather than argued:

  1. **THE BODY FACE.** Six runs have a cap SHORT while their x-height is TALL —
     opposite signs, which no `scaleY` can reconcile because it multiplies both.
     All six were built at their cap-landing `sy` and all six are much worse.
  2. **THE WORDMARK FACE.** Canonical S/H ink-width ratio 0.850 against Geist's
     1.002-1.009, Boxed 1.000, Tungsten 0.966.
  3. **THE THREE HELP-ROW LETTER WIDTHS** (0.866 / 0.8747 / 0.9136) — three
     different values are required to match canonical in a face that is not
     canonical's.
  4. **THE SHIELD AND GEAR ASSET RE-TRACES.** Refused four times each. The
     shield's fractional height error is SIX different numbers by azimuth, low at
     the shoulders and HIGH at the crown; the gear's cog is 28.2 px tall at every
     side azimuth where canonical runs 30.0-32.1. Canonical draws different
     curves, and no box or transform reaches a different curve.
  5. **CANONICAL'S TUNGSTEN IS A HEAVIER CUT (~478), AND UNEVENLY SO** — per-glyph
     mass 1.0126 / 1.0655 / 1.0662 / 1.1593. `display` alone is 19.3019 of the
     4.1139 whole-screen figure, 21% of n_over8, and is face-limited down to the
     mechanism.

Those five hold **36% of the screen's total error** and none of it is reachable
from this repository's assets. The remaining reachable residual is what grades
29-32 spent four rounds measuring and declining: everything left is either
refused by a landmark it would move the wrong way (rule 34), refused because
`n_over8` and `mean|d|` disagree (rule 71), rule-109 class where the geometry is
right and the metric declines it, or inside the lattice noise (rule 111).

**GRADING STOPS HERE.** Dispatching grade 33 would be asking a thirty-third
independent instrument to find what thirty-two could not, on a screen whose every
declared axis is now inventoried, swept, dead, degenerate or closed for cause.
Round 46 earned the right to be sceptical of a floor claim; this one carries the
evidence that claim was missing.

### ROUND 46 (GRADE 31, **A−**): THE FLOOR CLAIM IS REFUTED BY ONE CHARACTER, AND THE AXIS THAT DID IT HAD NEVER BEEN AUDITED

Pixels **4.1162 -> 4.1139** against a predicted 4.1139; n_over8 **90561 ->
90508** against a predicted 90508. All nine moved bands matched grade 31's
listed values exactly; **35 of 44 unmoved**, the count predicted. Gates 11/11,
3/3, 7/7. The change is **one character**: `--s5-orange:#FD4201 -> #FD4301`.

**IT IS WORTH MORE THAN ROUNDS 44 AND 45 COMBINED, AND BOTH HALVES OF THE FIGURE
TAKE IT** (-0.0023 and -53, where rounds 44 and 45 bought -0.0009 and -0.0014
with n_over8 -20 and flat). Every one of the nine moved bands is an
`--s5-orange` consumer.

**THE FILE'S OWN DOCSTRING HAD THE RIGHT NUMBER.** Line 68 reads *"d in [10,20):
(253.23, **67.04**, 1.14), confirmed by two raw interior samples at 65.7 and 66.5
green"* — and the token shipped **66**. The two point samples dragged the
rounding down and the plateau figure was right. That is round 29's shape for the
fourth time: a constant contradicting a measurement recorded a few lines above
it.

**THE ESTIMATOR IS THE POINT, AND IT IS THE SIMPLEST ONE THIS SCREEN HAS USED.**
An L1 argmin over canonical's **5x5-eroded** plate interior — no edge, no
antialias ramp, no unsharp ring can enter — over ~69,000 px. L1 rather than L2
**because the metric is L1** (rule 92a), so the argmin *is* the median. No
crossing, no coverage level, no fraction of peak, so rules 8, 90 and 106 cannot
enter at all. My own estimator, written independently, returned canonical's
plate at **R 253 / G 67 / B 1** on 68,277 px — grade 31's answer exactly.

**AND THE NULL CONTROL IS WHAT MAKES IT A FINDING RATHER THAN A FISHING TRIP.**
The same audit on PAPER over ~1.15M px returns **254/254/254 at gain 0.0000 on
both images**. An instrument that finds a spare unit on the plate and none on the
paper is measuring the plate.

        landmark                     shipped   round 46   required
        render plate G (eroded)         66        67      canonical's 67
        render plate R / B              253/1    253/1    must hold
        paper argmin, both images     254/254/254  unmoved   null control

Bracketed at +/-1 on all three axes and no neighbour scores as well: G 68
+0.0047, R 252 +0.0092, R 254 +0.0026, B 2 +0.0034.
...
against the plateau's 69,021. Rule 92(b) needs more than that to justify two
roles where the file has one.

### WHY FORTY-FIVE ROUNDS MISSED IT — AND IT IS THE THIRD TIME THIS EXACT THING HAS PAID

**Rule 71 closed the LUMINANCE axis and was read ever after as closing COLOUR.**
It does not. Round 46 is the third finding in seven rounds that came from an axis
the recipe **declares** and no round had **swept**:

    grade 25   `letter-spacing`   declared, emitted, never swept in 40 rounds
    grade 26   `sy` as a 2-D grid  swept as a bare 1-D sweep, which measures nothing
    grade 31   the colour tokens   ~2,000 candidates of geometry, no colour audit

See rule 110. The ~2,000 built candidates across grades 24-30 covered `sy`, `ty`,
`tx`, `letter-spacing`, `word-spacing`, `scaleX`, `weight`, eleven mark boxes as
coupled grids, ten stroke widths at a 0.12 step, the blitter and the anisotropy —
**all of it geometry**, and the one axis that was pure appearance sat untouched.

### FINDING 2 IS RULE-109 CLASS AND IS NOT PRESCRIBED

`--s5-graphite`'s blue is 4 units too high — `#454751` against a measured
`#45474D`. Three estimators sharing no parameter:

  * **ramp regression** (B on R over mid-ramp pixels; paper cancels, and blur and
    unsharp overshoot are per-channel linear so they cancel too). Calibrated on
    **six known `#000000` runs in canonical's own raster**, which return 0.9988 to
    1.0033 against a true 1.0000 — bias +0.0006, spread +/-0.0033. The seven
    graphite runs read **0.9571 mean (sd 0.0037)** against the token's 0.9355:
    a gap of **6.8x the calibration spread, same sign on all seven**. Green reads
    +0.0019, INSIDE the calibration — **only blue is wrong**.
  * **per-pixel ratio profile vs ink level** — canonical's graphite plateaus at
    0.954-0.956 for R-ink >= 130 while the five black controls read **exactly
    1.0000 in every one of those same bins**.
  * **conserved ink mass per channel** (an unsharp kernel sums to 1), small-run
    bias calibrated on the black runs.

Three answers: B = 77.0 / 77.2 / 78.4. Shipped 81 is outside all three.

**And the objective refuses it.** Swept at 1-unit resolution, `n_over8`'s argmin
is the derived `0x4D` **to the unit** — 90395, cleanly bracketed both sides, all
seven graphite runs improving — while **mean|d| declines it by +0.0008**, its own
argmin sitting 8-12 units bluer than canonical measures at n8 **+277 / +544**.
Rule 71's signature on a third axis. Recorded with its exact value, not
prescribed, on round 45's chevron precedent. **If n_over8 is ever given standing
symmetric with the refusals rule 71 has made at n8 +17, +19, +23 and +48, this is
the first thing to take.**

### THE INSTRUMENT REPRODUCED A RECORDED CHANGE INCLUDING TWO n_over8 COUNTS

Rule 105 part 2, in the family it prescribes in — literal colour strings. Grade
31 reverted round 35's joint (height, `fill`) change, recovering the pre-35
constants from `git show 7f43ec7^:` **rather than guessing them**, and got back
five bands and two counts exactly: hdrRule 0.6514, rule1 0.8536, rule2 0.5189,
rule3 0.9089 with n_over8 172, rule4 0.6528 with n_over8 56.

It also predicted this build's own numbers from the control raster and canonical
alone, before scoring — plate band **-0.0270 predicted / -0.0308 measured**,
whole **-0.00205 / -0.00232**, 88% on both, the remainder being the antialiased
rim the prediction deliberately does not model.

**And the stale-`variant/` warning is now four for four.** Grade 31 re-seeded from
`git show HEAD:` itself and md5-verified against `src/`; its control render's md5
equalled the shipped render's.

### THE BACKTICK TRAP FIRED FOR THE SIXTH TIME, ON ME, WITH THE GUARD IN VIEW

Writing this round's comment I put backticks around `--s5-orange` **inside the
`COLOURS` template literal, four lines below the guard telling me not to**. Rule
98 is amended with the honest count: the guard has now been present for
occurrences four, five and six and has prevented **none** of them, while `tsc` as
its own step has caught **all six**, every time in about a minute. Keep the guard
as documentation. **The control is running the type checker before the build, on
every edit, without exception.**

### ROUND 45 (GRADE 30, **A−**): THE ANISOTROPY THIS FILE LEFT OPEN FOR TWENTY-TWO ROUNDS, AND A DEFECT MEASURED EXACTLY, CORRECTED EXACTLY, AND REFUSED BY ITS OWN BUILD

Pixels **4.1176 -> 4.1162** against a predicted 4.1162; n_over8 **90561 ->
90561** (flat, so rule 71 holds); helpIcon1 6.2339 -> **5.9123**, predicted
5.9123. **43 of 44 bands byte-identical**, the count predicted.
Gates 11/11, 3/3, 7/7.

**THE DEFECT IS THE ONE `Icon`'s OWN COMMENT HAS BEEN NAMING SINCE ROUND 22.**
That note ends: *"THE ANISOTROPY IS THEREFORE STILL OPEN: horizontal strokes stay
~11% thinner than verticals on every anisotropic mark … the honest candidates
are per-axis path outlines, or making the mark boxes isotropic."* `Icon` emits
`scale(sx,sy)` with ONE `strokeWidth`, so one `sw` produces TWO drawn widths —
`sw*sqrt(sx/sy)` vertically and `sw*sqrt(sy/sx)` horizontally. On `helpMark1`
they are **11.7 percentage points apart** and canonical draws one width.

        mark        box aspect   vert R/C   horz R/C   spread   canon's own v/h
        helpIcon3      0.9918     1.0024     1.0075    -0.5 pp      0.9971
        back           1.0000       —        0.9982       —           —
        diffMark       1.1156     1.0083     0.9597    +4.9 pp      0.9965
        helpIcon2      1.0728     1.0407     0.9836    +5.7 pp      1.0228
        gear           1.0588     1.0743     0.9859    +8.8 pp      0.9579
        helpIcon1      1.1261     1.0482     0.9308   +11.7 pp      0.9519
        plateMark      1.0926     1.0725     0.8967   +17.6 pp      0.9356

**The render's per-axis ratio tracks the BOX ASPECT to 0.06%; canonical's own
sits at 0.94-1.02. And the control is in the raster rather than in an argument:
the two ISOTROPIC boxes on this screen — `back` 50.5x50.5 and `helpIcon3`
66.2x66.75 — land BOTH axes to 0.8%.**

**WHY TWENTY-TWO ROUNDS OF SWEEPS COULD NOT SEE IT.** Inner AND outer crossings
on all four walls separate a translation from a width (rule 103), and they say
the centrelines are LANDED — three of four inside 0.02 px — while the widths are
wrong with **OPPOSITE SIGNS ON THE TWO AXES**: top -0.229, bottom -0.248, left
+0.125, right +0.128 (my own estimator; grade 30 read -0.212 / -0.262 / +0.131 /
+0.134). **No `sw`, no box and no nudge can reach that, because all three move
both axes together**, and `sw 3.15` already sits at the geometric mean.

**THE FIX IS ALGEBRAICALLY GEOMETRY-PRESERVING, WHICH IS WHY IT IS SAFE.** Scale
uniformly by `sy` and pre-scale every path x by `c = sx/sy`: `x_final = bx +
sy*(x*c) = bx + sx*x`, so no ink moves and only the stroke changes;
`strokeWidth = sw/sy` then draws `sw` in both axes. Every arc in the path has
x-rotation 0, so scaling x takes `(rx, ry, 0)` to `(rx*c, ry, 0)` exactly. I
verified all three rewritten paths term by term before building.

**AND `sw 3.19` WAS DERIVED, NOT SEARCHED.** Under `iso` all four walls draw at
`sw`, so the total error against canonical's {3.073, 3.088, 3.215, 3.266} is
**FLAT at 0.320 for any sw in [3.088, 3.215]** and rises outside it. Inside that
band the 0.248 px ladder collapses the choice to two reachable states, and only
the upper puts the top bar on the right side of canonical. **`sw 3.32` is the
objective's argmin and is refused** — outside the licensed band, wall error 0.275
-> 0.558. The measurement over the argmin, for the fifth time in two rounds.

**BOTH REFUTING MEASUREMENTS WERE TAKEN BEFORE SCORING, AND THE SECOND WAS A
BACK-OUT CONDITION RATHER THAN A CAVEAT.** All four walls landed on their
predicted values to three decimals:

        wall     shipped   round 45   target
        top       3.003     3.335     3.335 +/- 0.03
        bottom    3.003     3.168     3.168 +/- 0.03
        left      3.220     3.114     3.114 +/- 0.03
        right     3.207     3.104     3.104 +/- 0.03
        total |width error|  0.731 -> 0.229

The walls thinned 0.106 and 0.103 against a required >= 0.08, which is what
proves the uniform transform actually TOOK (rule 99 — round 37 wrote a property
onto an element that could not inherit it and measured a null experiment). And
the top bar reached **3.335 against a required >= 3.28**: below that rung the
same edit still improves the measurement and **loses 0.18 of band**, and the
instruction was to back it out rather than re-tune it. It cleared.

**IT IS OPT-IN PER MARK, AND THAT IS RULE 108's DISCIPLINE APPLIED.** The sign of
the iso gain is ordered by **canonical's own v/h ratio**: it pays on the
horizontal-heavy marks (helpMark1 0.952, plateMark 0.936, gear 0.958) and makes
the neutral ones WORSE (diffMark 0.997 -> +0.0004/n8 +23; helpMark2 1.023 ->
+0.0015/n8 +63, the worst on the screen). Of the three that gain, only helpMark1
passes rule 71. **The emitter defect is general; the prescription is not** — a
mark-by-mark measurement, never a rewrite per provenance.

**AND THE MECHANISM WAS SEPARATED FROM THE TONE.** Under `Icon` the geometric
mean of the two drawn widths is `sw` in BOTH modes, so iso@sw and aniso@sw carry
the same ink mass and differ only in its distribution. At matched `sw`, helpIcon1
anisotropic reads 6.2339 / 6.2348 / 6.4370 at 3.15 / 3.25 / 3.35 — flat-to-worse
everywhere — against iso's 6.4148 / 5.8524 / 6.2030. **The gain is the
redistribution, not the weight.**

### THE CHEVRON ASPECT: MEASURED TO 0.06%, CORRECTED EXACTLY, AND REFUSED BY ITS OWN BUILD

Grade 30's second finding is the most interesting thing in the round and it
ships nothing. A lucide `m9 18 6-6-6-6` arm drawn through `scale(sx,sy)` has
`dcol/drow` **exactly** `bw/bh`, so a coverage-weighted per-row centroid line
reads the box aspect and is immune to translation, to stroke width and to the
width ladder. Six arms: render mean **0.9375** against the emitter's own
prediction 0.93694 — **0.06%** — and canonical **0.8921 +/- 0.0083**. Ratio
**1.0509**. A second statistic sharing no parameter (arm centreline extrapolated
to tip and vertex) gives tip **-0.261** and vertex **+0.283** — opposite signs at
the two ends of one arm, so a SIZE error by rule 103, at **5.04%** against the
slope's 5.09%, with the centre landed at +0.011.

The correction `bw 41.6 -> 39.60, tx 1.8 -> 2.80` then landed **every landmark it
predicted**: slope 0.9378 -> **0.8933** (predicted 0.8925 +/- 0.010, canonical
0.8956), tip -0.252 -> **+0.031**, vertex +0.251 -> **+0.007**, half-size +0.251
-> -0.012, translation held. Estimator-D arm width 2.5576 -> **2.5139** against
canonical 2.5099 — **the aspect error was also the width error**, one cause
explaining both symptoms, which is the strongest form a diagnosis takes here.

**And the objective declined it: whole 4.1176 -> 4.1179, +0.0003.** The metric's
own argmin on that axis is `bw 41.0`, which leaves the drawing most of the way
wrong (half-size +0.169). Grade 30 took the refutation (rule 80) and recorded the
defect with its exact correction and its stated cost rather than shipping it or
dropping it. **See rule 109 — this is a new category and it is a signal about the
METRIC, not about the screen.**

### THREE SMALLER THINGS WORTH KEEPING

**A THIRD STALE `variant/`.** Grade 30 found g29's inherited copy carrying round
44's edits and re-seeded from `git show HEAD:` itself, md5-verifying against the
working tree. That is three graders in a row; the brief has warned about it twice
and the warning is now load-bearing. Its control build's md5 **equalled the
shipped render's** rather than merely scoring 0.0000.

**A RULE-107 RE-TAKE THAT CAME BACK NULL, AND THAT IS THE POINT.** `helpMark2`'s
`sw` was swept by grade 28 against the ROUND-42 raster — before rounds 43 and 44
moved that mark's hands, `rx`, `cy` and `ry`. Re-swept on the current composite:
2.88 -> 5.9211, 3.00 -> 5.6760, **3.12 -> 5.1229 (shipped)**, 3.24 -> 5.5995,
3.36 -> 5.9405. **Control is argmin, cleanly bracketed both sides.** The refusal
survives its re-take. Rule 107 does not say old refusals are wrong; it says they
are *unverified*, and this one verified.

**AND TWO FACTS RECORDED SO THE NEXT ROUND DOES NOT RE-DERIVE THEM.** The stroke
anisotropy is real on six marks and reachable on one. And the chevron arms differ
in ANGLE, not only in width — which is why five rounds of `sw` sweeps on that
mark kept finding gains that were the ladder.

### ROUND 44 (GRADE 29, **A−**): RULE 107 RAN BACKWARDS AND LICENSED AN EDIT A PREVIOUS ROUND WAS RIGHT TO REFUSE — AND THE `k` AUDIT ENDED "THE DERIVATION IS WRONG AND THE VALUE IS RIGHT"

Pixels **4.1185 -> 4.1176** against a predicted 4.1176; n_over8 **90581 ->
90561** against a predicted 90561; helpIcon2 5.3367 -> **5.1229**, predicted
5.1229. **43 of 44 bands byte-identical**, the count predicted. Gates 11/11,
3/3, 7/7. One edit: `ry 4.500 -> 4.546`.

**RULE 107 IS SYMMETRIC, AND THIS IS THE HALF NOBODY HAD USED.** The rule was
written because a landmark contaminated by a sibling defect made an edit look
justified when it was not. Here it runs the other way. Round 43 **refused `ry`
for a correct reason**: on that raster the dial's two vertical edges read +0.247
and +0.303, BOTH POSITIVE, which is a translation, and `ry` would have landed the
top while driving the bottom to +0.56. Correcting `cy` removed the translation,
and what remains on the composite is a SIZE — T_out +0.063 against B_out -0.173,
opposite signs. **The refusal was right for its raster and wrong for this one.**
An edit refused in an earlier round is not closed; it is closed *against the
raster that existed then*.

All four outer edges point inward (L +0.034, R -0.138), so the ring is uniformly
~0.1 px small and only the VERTICAL clears the 0.248 device-px ladder — which is
why `ry` moves alone. **My own refutation reproduced grade 29's honest caveat
exactly**: B_out -0.135 -> **-0.029** (inside +/-0.10), T_out +0.064 ->
**+0.064, unmoved**, R_out control 0.023 (tolerance 0.03). Half the predicted
response was eaten by the ladder, and the grade said so before the build rather
than after.

### THE `k = 2.2257` AUDIT IS CLOSED, AND ITS ANSWER IS "DO NOT TOUCH IT"

I named this audit in the brief as a fact: the wrong scale still sat at
`Marks005:608` and `:617` as the derivation basis for other constants, and grade
28's closing sentence had called for exactly this sweep. **The answer is not what
either of us expected.**

The one surviving k-derived constant is `cx = 17.5 + 2.77/k = 18.74`. The
consistent correction is `cx = 17.5 + 2.77/sx = 18.5027`, a **0.655 px** move.
Grade 29 took the refuting measurement FIRST: a 0.655 px `cx` error must move
BOTH horizontal outer edges the same way by 0.655, and the ring's measured
horizontal translation is **-0.052 px**. Built anyway: `cx 18.5027` scores
**+0.0051 / n8 +20**, taking helpIcon2 to **6.4597 — worse than before round
43** — while 18.68 and 18.80 straddle 18.74 at +0.0003 / +0.0004.

**`cx 18.74` is at argmin. The derivation is wrong and the value is right**,
because its input — 2.77 px from a circle fit made when the ring was 20%
oversized and merged with the envelope — was wrong by almost exactly the
compensating amount. **A grader completing the audit "by symmetry with `cy`"
would have given back more than round 43 won.** See rule 108.

A second single-derived scale was found and is also clean: `Marks005:339,357`
uses `k = 2.909` — itself an ink-extent statistic — to convert BOTH a vertical
and a horizontal on `diffMark`, whose true scales are 3.2000 and 2.8684. Measured
on the composite: bottom bar row **-0.116 px**, bar end col **+0.274 px**. Both
landed. Nothing to prescribe.

### THREE REFUSALS WORTH 0.0213, EACH NAMING WHAT IT HIDES

**`shield bh 104.0`, -0.0110, still the largest gain on the canvas.** Grade 27
refused it on the interior tick; grade 29 refused it again on evidence grade 27
did not have, and **also showed grade 27's stated reason does not hold although
its verdict does**. The tick is a DIAGONAL, so a column-band reading of it spans
**-0.45 to +1.23 — 1.68 px of spread on a quantity of ~0.5** — rule 106's own
definition of a broken estimator. The sound refutation is two other things:
  * *Azimuth.* A real box-size error scales the whole drawing, so the FRACTIONAL
    height error must be ONE number. It is six: **-0.19% / +0.39% / +2.07% /
    +1.68% / +0.58% / -0.36%**. The top is low at the shoulders and HIGH at the
    crown — wrong in both directions at once, which no (bh, ty, bw, tx) reaches.
  * *Orthogonal.* `bh` scales only y, so outline WIDTH at fixed rows is
    untouchable by it. The candidate's single largest win is at rows 1710-16 —
    **grade 18's measured fat tip, +2.314 -> +0.352** — while the three bands
    above get **37% worse**. It buys the objective by pulling the drawing up
    until the fat tip leaves the frame.

**`gear bh 52.3 / ty -1.2`, -0.0092.** Grade 29's geometry solved (52.28, -1.22)
BEFORE the grid and the grid's argmin came back exactly (52.3, -1.2) — and it
refused its own solved value. The render's cog is **28.2 px tall at every side
azimuth, a constant**, where canonical runs **30.0 to 32.1**: canonical draws
taller lobes. The edit lands one landmark and drives four bottoms further out,
two of which were landed, while closing only 36% of the real error. Grade 19's
asset verdict confirmed with a rule-90-compliant instrument.

**`helpMark1 ty 0.41`, -0.0011.** It lands the envelope (-0.113 -> +0.059) and
drives the check vertex — the most stable landmark in the mark, spread 0.001 —
from +0.382 to +0.515, **predicted at +0.496 before the build and hit to 0.019**.
The mark's two features disagree IN SIGN and the check's displacement is
non-uniform along its arms (+0.25 / +0.85 / +1.21), so it is a shape difference
no box lever reaches; `ty` merely moves the ink-weighted average. Also `ry 4.580`
— the objective's own argmin, **74% more than the measurement asks**, driving
T_out to -0.095, past canonical and larger than it started, for 0.0004 more.
**The measurement wins over the argmin, four times in one round.**

### AND IT APPLIED RULE 80 TO ITSELF, TWICE

Its first horizontal probe returned a slope of **exactly 0.0000** over five
builds. The lever was not dead: its search span ran past the bar's end into the
pencil, so the "trailing crossing" was the pencil's edge every time. **A
dead-flat response to a lever the emitter says must work is a bug in the probe,
not a finding** — and every landmark it took afterwards is indexed into a
complete crossings list rather than taken as "first crossing in a span". Its
first coverage LEVEL was also wrong (162.77 against the true paper/ink midpoint
127, because these marks draw in `#000`), and it re-took every reading with a
level-sensitivity column attached.

### TWO STALE ASSERTIONS FIXED — WRONG TEXT, RIGHT PIXELS

Grade 29's rule-107 survey re-took all eleven marks on the current composite and
found every body edge inside 0.25 px except the two asset marks. It also found
two comments in `Marks005.tsx` that are simply false now: one still quoting
`diffMark`'s box as 76.8 x 68.5 with 12.1% anisotropy (round 40 moved `bh`; the
true figures are 2.8684 and 11.56%), and one still asserting helpMark2's body is
"1.13 device px too tall" from a pre-correction reading, when all four of its
edges are now landed to 0.04 px at every level from 89 to 178.

**Both are corrected in this round, and the reason is rounds 41-43.** Three
separate constants in this recipe were wrong because a stale number upstream of
them was believed: `lede2`'s ascender, `helpIcon3`'s height, the clock hands'
offset. **A comment that states a measurement is an input to the next round's
arithmetic, and this file has now paid four times for treating prose as
decoration.**

### ROUND 43 (GRADE 28, **B+**): A CONSTANT CONVERTED WITH A SCALE THE SAME FILE PROVES WRONG, FORTY LINES ABOVE IT

Pixels **4.1236 -> 4.1185** against a predicted 4.1185; n_over8 **90604 ->
90581** against a predicted 90581. helpIcon2 6.3798 -> **5.3367**, chev1 -> 
**1.2326**, chev2 -> **1.2263**, chev3 -> **1.1838**. **40 of 44 bands
byte-identical**, the count predicted. Gates 11/11, 3/3, 7/7.

**THE DEFECT IS A CONSTANT DERIVED WITH A SCALE THIS FILE ALREADY PROVED WRONG.**
`Marks005.tsx` justifies the clock hands' position with *"0.45 units is 1 device
px at k = 2.2257"* — and roughly forty lines above it sits *"k is neither sx
(2.7625) nor sy (2.5750), and every number derived from it inherits the error."*
One device px horizontally is `1/sx = 0.3620` units, so round 23's intended 1 px
landed as **1.243 px**. And the 1 px it was correcting was itself an **extent**
reading (cols 69..75 against 68..74 — rule 90) of a true 0.37 px offset. **Wrong
input, wrong conversion, self-consistent downstream, twenty rounds inside the
fourth-worst window on the canvas.** That is the third instance of this shape in
five rounds: round 33's `lede2` ascender (half the figure, eight rounds), round
20's `helpIcon3` height (three times the figure, twenty-one rounds), and now this.

**THE SAME ELEMENT CARRIED TWO DIFFERENT DEFECTS AND THE PAIRED EDGES SEPARATED
THEM.** The dial's horizontal edges point INWARD at each other (right wall
-0.337, lower-left arc +0.194), which is a size error — and it is why `cx` reads
argmin in both directions. Its vertical edges move the SAME way (+0.247, +0.303),
which is a translation, with the outer height already right at +0.056. So `rx`
and `cy` are two findings on one `<ellipse>`, and `ry` is refused: `ry 4.60`
scores -0.0018 and lands `T_out` while driving `B_out` from +0.303 to +0.56.
**One element, two defects, three axes, and only the paired-edge reading tells
them apart** (rule 103, working exactly as intended).

### THE GRADER REFUTED ITS OWN TARGET HYPOTHESIS BY GEOMETRY, BEFORE ANY BUILD

I pointed grade 28 at the `sw`/box coupling — `Icon` divides by `sqrt(sx*sy)`, and
round 42 had just moved four boxes, so the drawn weights looked like they must
have drifted. **The emitter says otherwise without a build.** A 45-degree arm's
perpendicular width is `sqrt(2)*sw*sqrt(sx*sy)/sqrt(sx^2+sy^2)`, which is
**second order** in the aspect: the chevron box 43.6x43.2 -> 41.6x44.4 moves it
2.5985 -> 2.5974, **-0.04%**. The recipe's own note that "the box change
re-weights the stroke ~3.7%" is true of an AXIS-ALIGNED stroke and false of this
drawing. helpMark3's box is near-isotropic either side: +0.018 px, inside one
ladder step. **Round 42 left no stroke-weight debt, and the whole `sw` axis is
argmin on eight of ten marks.** A brief's lead refuted for the fourth time in
five rounds — and this one refuted by reading the emitter rather than by
building, which is cheaper and stronger.

**AND IT APPLIED RULE 80 TO ITSELF ON A LEDGER DISCREPANCY.** Its `gear` sw revert
returned 8.6695 where this file records 8.6972. It did **not** write up a ledger
error: it read `43b984e^`, found gear's pre-round-37 `sw` was **2.5** and not the
3.0 it had assumed, re-ran, and got 8.6972 exactly. **The ledger was right and
its assumption was wrong.** Three of three reverts then reproduced round 37's
recorded numbers — rule 105 part 2, in the `Icon`/`sw` family it went on to
prescribe in.

### AND ITS OWN FIRST ESTIMATOR WAS WRONG, WHICH IS THE MOST USEFUL PART

On the chevron stroke a 50%-**of-local-peak** crossing said canonical's arm is
2.1858 against the render's 2.6024 — a **19% excess**. That is wrong, and the
mechanism is worth a rule: **canonical's core overshoots to coverage 1.194**
because it carries an unsharp mask, and its flanks are sharpened, so a
fraction-of-peak level sits too high on canonical and reads a thin stroke narrow.
Three levels of that estimator returned **19% / 13% / 6%** — a spread wider than
the quantity being measured, which is the signature of a broken estimator rather
than a noisy one. See rule 106.

The replacement has a linearity argument: the **row-cut integral / sqrt(1+m^2)**,
pedestal removed from the far field. An unsharp mask is linear and zero-sum, so
it conserves a row-cut integral taken wide enough to hold the halo, and
resampling conserves it too — so it is unbiased ACROSS the two rasters, which is
the property that matters. Validated against geometry (Icon predicts 2.5972; it
reads 2.5896, 0.3%) and for linearity (slope **1.00** over twelve builds). True
excess **+0.0688 +/- 0.0226 sem over six arms**, 3 sigma, corroborated by window
ink mass at 1.029 putting the target at 2.527 against the estimator's 2.531.

### THE SUPERSAMPLING LADDER, MEASURED

The render's coverage takes only **0 / 63 / 127 / 190 / 254** — Skia's
supersampling blitter, the same one grade 24 modelled for `diffBtn` — so an
axis-aligned stroke edge sits on a **0.2480 device-px ladder** and `sw` has
**~0.50-wide dead zones**: `back` is pixel-identical on its bar across sw
2.84-3.16, 3.20-3.32 and 3.36-3.64. Every `sw` sweep in this round stepped at
0.12, finer than half a cell. **A sweep coarser than that on this axis measures
the ladder, not the mark.**

### THE GREEDY COMPOSITE WAS BUILT AND DECLINED IN FULL

`chev 2.40, rx 4.32, cy 18.20, hands y 16.50` builds to **4.1167 / n_over8
90544** — **0.0018 and 37 pixels better than what was prescribed** — and every one
of its four coordinates drives a landmark past canonical:

    chev sw 2.40   argmin of the whole axis, -0.0013/n8 -29, FOUR TIMES the
                   prescribed gain: a 9% thinning against a 2.7% measured excess.
                   Drives chev1's upper arm, landed at -0.0007, to -0.2750.
    rx 4.32        objective argmin (-0.0044 against 4.246's -0.0039). Drives
                   LL_out from +0.194 to -0.334, 1.7x further out than it started.
    cy 18.20       -0.0019 against 18.263's -0.0016. Overshoots the measured
                   0.275 px.
    hands y 16.51  -0.0004, and once the ring is corrected the hands' ROW is
                   already landed at +0.043; this drives it to +0.398.

**The last one is the round's sharpest process point.** It was caught only
because the landmark was re-taken on the COMPOSITE — on the isolated edit the row
still read -0.386 and the change looked justified. **A landmark measured against
a defect that a sibling edit is about to fix will lie to you.** Three graders in
a row have now declined their own largest gain on interior-landmark evidence.

### ROUND 42 (GRADE 27, **B+**): RULE 104's COUPLING CLASS EXTENDS TO `Icon`, AND THE GRADER REFUSED ITS OWN LARGEST FINDING

Pixels **4.1305 -> 4.1236** against a predicted 4.1236; n_over8 **90670 ->
90604** against a predicted 90604. helpIcon3 7.2985 -> **6.2137**, chev2 1.5480
-> **1.2610**, chev1 1.5005 -> **1.2905**, chev3 1.3690 -> **1.2071**. **40 of 44
bands byte-identical**, the count grade 27 predicted. Gates 11/11, 3/3, 7/7.

**RULE 104's COUPLING CLASS EXTENDS TO `Icon`/`markBox`, AND IT EXPLAINS THREE
REFUSALS RATHER THAN ONE.** `Icon` emits `<g transform="translate(bx by)
scale(bw/24, bh/24)" strokeWidth={sw/sqrt(sx*sy)}>`, so `bh` scales the drawing
about the box's **TOP** edge, `bw` about its **LEFT**, and both **re-weight the
stroke**. A bare `bw`/`bh` edit therefore moves, resizes AND re-weights at once —
and **every mark-box size verdict in this recipe was a bare edit judged on an ink
extent with no compensating nudge**: helpIcon3 ("2 px of symmetric height …
reverted"), gear ("1 px narrow on the left … reverted"), helpIcon2 ("+1 on both
columns … reverted"). Rule 90 refuses the statistic and rule 104 refuses the
sweep, and between them they left 1.08 of band on helpIcon3 for twenty-one rounds
under a line saying the axis was closed. **Same shape as round 41's `lede2`
ascender, one emitter over.**

**D1's TRUE FIGURE IS A THIRD OF WHAT THE REFUSAL MEASURED.** Round 20 refused a
**2.25 px symmetric** change; the real error is **0.75 px asymmetric**, appearing
as +0.69 at the bottom and +0.05 at the top because the pivot is the box top.
Right direction, three times the magnitude, no `ty`. **And the objective cannot
set this value on its own** — its valley is a DIAGONAL RIDGE from (66.2, 0.45) to
(66.85, 0.10), flat at -0.005 along its whole length, so the whole-screen metric
cannot separate the height from the nudge. The size was set by the estimator and
only confirmed by the metric, which is the correct order and worth naming.

**THE GRADER REFUSED ITS OWN LARGEST FINDING, AND THE REFUSAL IS THE BEST WORK IN
THE ROUND.** `shield bh 105.2 -> 104.0` scores **-0.0110 whole, n_over8 -93,
shield 8.5019 -> 7.0003** — larger than everything grade 27 prescribed combined.
It passes every EDGE test, including both of round 40's D1 landmarks to three
decimals. **It fails the interior one.** The shield's tick — rows 1660-1700, cols
88-128, between the limbs, an interior feature no outline change should touch —
is landed on the shipped raster at +0.215 SSD and the shrink drags it to -0.490,
within 0.09 px of the -0.576 the geometry predicts. A box genuinely too tall puts
every interior feature proportionally low; here only the outer BOTTOM is low, and
that is grade 18's measured fat tip. **Shrinking the box hides the tip by pulling
the whole drawing up.** Rule 34, refuted by an instrument grade 18 did not have,
and grade 18's closure stands. `gear bw` was refused too (-0.0025): at its best
cell the stretch it claims to fix still reads +0.958 of the shipped +2.354, and
the three estimators want 50.9 / 52.1 / 51.5 — a 1.4 px spread with no fourth to
break it (rule 102). **Total refused: 0.0135, twice what was prescribed.**

### AND MY OWN REFUTATION PARTLY MISSED, WHICH IS RECORDED AS A MISS

Rule 102's order was kept — all eight landmarks measured on the built capture
before `report005`. Seven of the eight are unambiguous:

        landmark                    shipped                 round 42        required
        helpIcon3 bottom  +0.588/+0.517/+0.591   -0.065/+0.002/-0.063   |x| < 0.20
        helpIcon3 top     +0.080/-0.059/-0.090   +0.097/-0.024/+0.012   |x| < 0.15  (hold)
        chev1 h stretch            +0.564                 +0.045        < +0.20
        chev2 h stretch            +0.667                 +0.131        < +0.20
        chev3 h stretch            +0.501                 -0.071        < +0.20
        chev2 v stretch            -0.758                 -0.126        > -0.25
        helpIcon2, shield controls    -            UNCHANGED, digit for digit

**chev1 and chev3's vertical stretch land at -0.333 and -0.263 against a required
> -0.25 — outside the tolerance grade 27 stated.** Grade 27's own build reported
-0.209 / +0.028 / +0.007, all passing. The gap is an estimator OFFSET, not a
build discrepancy: mine reads the SHIPPED chevrons 0.08-0.13 more negative than
its does (-0.966/-0.758/-0.849 against -0.89/-0.63/-0.74), and the after-values
inherit exactly that offset. The correction closed 0.633 / 0.632 / 0.586 of a
0.966 / 0.758 / 0.849 deficit — about three quarters — on both instruments.

**So the honest statement is: the direction and most of the magnitude are
confirmed on two independently written estimators, and on mine the chevrons
remain ~0.3 px short vertically.** That is a residual, stated with its numbers
rather than smoothed into a pass, and it is not chased further because the
objective's argmin is where it is. It is the second time in three rounds that my
estimator and a grader's have disagreed at the third decimal; both times the band
was unambiguous and both times the disagreement was in the instruments' offsets,
not in the claim.

### THE GRADER VALIDATED ITS RIG AGAINST HISTORY, IN THE FAMILY IT PRESCRIBES IN

Grade 26 rebuilt round 33's `lede1`. Grade 27 did the same thing one emitter
over, because its target was `MARK_BOXES` and not `RUNS`: reverting `diffMark` to
`bh 68.5, ty 0` on top of HEAD returns diffMark 8.3575 -> **9.0344** and diffBtn
3.8069 -> **3.8481**, which is verbatim what this file records for round 40, two
rounds back, through the emitter it intended to prescribe in. It then calibrated
its estimator on that same build: geometry says the edit should change `stretch`
by +0.270, and the measurement came back **+0.270 exact**. **Validate the
instrument in the family you are about to prescribe in, not merely on the screen.**

**AND IT REPORTED A BUG IN ITS OWN INSTRUMENT.** Writing the third estimator
caught a quantised modal colour leaving a constant ~130-unit pedestal under every
profile, which made that estimator return the SAME number for four different
rasters. Found and fixed before any prescription. A grader that reports its own
instrument bug is worth more than one that does not have any.

### EMPTY, AND THAT IS A RESULT

`helpIcon1`, `helpIcon2`, `plateMark`, `diffMark` and `back` have every box axis
landing inside the three estimators' own agreement on the shipped raster
(|stretch| <= 0.38, readings straddling zero). The two the brief flagged as never
measured are now measured and are **landed**: helpIcon1 vertical stretch
-0.045/+0.160/-0.001, plateMark -0.266/+0.180/-0.304. **There is nothing to
prescribe on them**, and plateMark's 4.5255 and helpIcon1's 6.2339 are not box
geometry.

### ROUND 41 (GRADE 26, **B+**): A BARE SWEEP IS THE WRONG INSTRUMENT FOR A COMPENSATED PAIR, AND A CORRECTION THAT LOOKED COMPLETE WAS HALF AN ERROR

Pixels **4.1483 -> 4.1305** against a predicted 4.1305; n_over8 **90833 ->
90670** against a predicted 90670. lede2 6.5651 -> **6.1525**, lede1 5.1769 ->
**4.9511**, resendLink 2.6799 -> **2.6163**. **41 of 44 bands byte-identical.**
Every figure grade 26's, to four decimals. Gates 11/11, 3/3, 7/7. Refuting
measurements taken BEFORE `report005`, per rule 102:

        landmark                     shipped    round 41     required
        lede1 height ratio (f=0.25)  1.0404     1.0003       1.000 +/- 0.008
        lede1 ink extent             (403,427)  (403,426)    exactly (403,426)
        lede2 height ratio           1.0592     1.0016       1.000 +/- 0.010
        help1, resendLab controls    -          UNCHANGED, digit for digit

My own estimator reproduced grade 26's shipped readings on both lede runs to
three decimals (1.0404 against its 1.0401; 1.0592 against its 1.0588) — two
independently written instruments, again.

**A BARE SWEEP IS THE WRONG INSTRUMENT FOR THIS AXIS, AND THAT IS THE ROUND'S
METHOD FINDING.** `transform-origin: 0 0` with `scaleX(s) scaleY(sy)
translate(tx,ty)` maps local y to `sy*(y+ty)`, so **`sy` moves the baseline as
well as resizing the run** — `sy` and `ty` are a compensated pair BY
CONSTRUCTION. A 1-D `sy` sweep called `display`, `lede1`, `resendVal`,
`resendLink`, `safe1` and `plateLab` argmin, and **every one of them gains once
`ty` is allowed to move with it.** Rule 91's masking, in the emitter's own
algebra. Grade 26 therefore gridded 5 sy x 7 ty per run at a `ty` step of 0.20
CSS px — finer than the ~0.46 CSS px raster plateau — so every reachable vertical
position is sampled at every `sy`. **Any earlier "argmin" on `sy` alone measured
nothing.** See rule 104.

**D1 IS A CORRECTION THAT LOOKED COMPLETE AND WAS HALF AN ERROR.** Built with
`lede2`'s `sy` REMOVED, the ascender ratio is **1.1294** and the x-height 1.1564.
Round 33's table records ascender **1.0611** and x-height 1.1402 — the x-height
figure is right and **the ascender was read at half its true value**. `0.9424` is
exactly `1/1.0611`, and `1.1294 x 0.9424 = 1.0644`, closing on the 1.0604 measured
on the shipped artefact. The arithmetic is self-consistent end to end, which is
precisely why nothing caught it for eight rounds: a wrong input produced a
correct-looking output and the run kept a 6% ascender excess under a line of the
recipe that said it had been fixed.

**D2 CAME WITH ITS OWN REFUTATION OF THE OBVIOUS IMPROVEMENT, AND RULE 97 KEEPS
ITS CALIBRATION POINT.** `lede1`'s prescribed 3.0% overshoots its measured 2.46%,
which grade 26 stated rather than hid: the cap ratio is pinned at 0.993 across
the whole raster row 0.970-0.976 and the next row up displaces the run bodily. It
then checked the tempting claim — that round 33's refusal had been an
uncompensated-`ty` artefact — and **found it false**: at `sy 0.9815` the cap ratio
is 1.0004, *geometrically exact*, and the band is still worse by 0.42 at every
`ty` from -0.5 to +2.0. Rule 80 applied by a grader to itself, which is the first
time that has happened on this screen.

**D3 IS HELD LOOSELY AND SAID SO.** `resendLink`'s cap is 0.68% tall — well below
rule 97's ~2.4% cutoff — and the change nonetheless re-rasterises measurably
(-0.0019, n_over8 -26) and lands the cap at 0.9958 with extents held exactly at
(811,835). Grade 26 graded its own finding LOW and predicted it might not survive
a different Chromium. Recorded that way in the recipe, with the instruction to
revert and record rather than diagnose if it ever goes the wrong way. **And my
own estimator could not judge it at all** — its f=0.25 crossing reads 0.80 with a
+4.4 top delta both BEFORE and AFTER the edit, which identifies the fault as my
window rather than the run, and is stated instead of being dressed up as a pass.

### THE BODY FACE IS NOW MEASURED ON SIX RUNS, NOT ASSERTED

Grade 26's third estimator splits the cap line off from the x-height line — both
as modal level crossings over tens of columns, so rule 90 does not apply — and
the result is the cleanest statement of the face problem this file has:

        run          cap ratio    x-height ratio
        lede1          1.0246        1.0829
        lede2          1.0604        1.0820
        resendLab      1.0294        1.0995
        resendLink     1.0069        1.0713
        diffLab        0.9636        1.0216
        help1          0.9434        0.9820
        help2          0.9597        1.0226
        help3          0.9468        1.0327
        safe1          0.9732        1.0233
        safe2          0.9800        1.0505
        plateLab       0.9700        1.0250

**For diffLab, help2, help3, safe1, safe2 and plateLab the cap is SHORT while the
x-height is TALL — opposite signs, which no `scaleY` can reconcile** because it
multiplies both. Built at their cap-landing `sy`, all six are much worse (+0.0208,
+0.0313, +0.0109, +0.0355, +0.0159, +0.0157). Only `lede1` and `lede2` have both
landmarks tall together, and only they have a value that lands the cap — which is
why exactly those two were the findings. The recipe's existing help2/help3
verdict now extends to four more runs **with build evidence rather than
inference**, and it sharpens the NEEDS KEVIN body-face decision from "the stem
ratio is wrong" to "the x-height/cap PROPORTION is wrong, and it is unreachable
by any transform this recipe can emit."

### ELEVEN GAINS REFUSED, WORTH 0.0575 OF WHOLE SCREEN

Every one drives a cap that is already short — or exactly landed — further from
canonical:

    display    sy 0.99   -0.0129 / n8 -308   cap 1.0049 -> 0.9953, extent (229,353) -> (230,353)
    safe2      sy 0.8502 -0.0111 / n8 -128   cap 0.9800 -> 0.9486
    safe1      sy 0.9032 -0.0078 / n8  -74   cap 0.9732 -> 0.9081, baseline lifted 1 px
    diffLab    sy 0.950  -0.0067 / n8  -71   cap 0.9636 -> 0.9401
    help2      sy 0.985  -0.0062 / n8  -78   cap 0.9597 -> 0.9480
    resendLink sy 0.9372 -0.0059 / n8  -43   cap 1.0069 -> 0.9704  (CROSSES 1.000)
    help3      sy 0.98   -0.0032 / n8  -40   cap already 5.3% short
    resendVal  sy 0.96   -0.0026 / n8  -20   row ratio 1.0030 (exact) -> 0.9613
    plateLab   sy 0.9216 -0.0011 / n8  -17   cap 0.9700 -> 0.9486

`display` is the sharpest refusal available anywhere on this screen: **the single
largest metric gain on the canvas**, and it takes the cap from exactly canonical's
extent to one row short — verbatim the trade this file already declined at
`80.63/+1.65`. Same defect, new lever, refused again. `resendVal` is rule 90 in
miniature: its 1-row EXTENT improves while its interior ratio goes from exactly
1.0030 to 3.9% short.

Argmin on the full (sy, ty) grid with nothing to take: `wordmark`, `didnt`,
`help1`, `resendLab` — and `resendLab`'s cap is 2.9% tall with its x-height 10%
tall and STILL no cell beats the control. Stated as a residual.

**THE `sy` AXIS IS NOW CLOSED ON ALL FIFTEEN BODY RUNS as a 2-D (sy, ty) grid**,
with each candidate's cap-landing value built explicitly. Do not re-open without
a new estimator.

**AND THE INSTRUMENT REPRODUCED HISTORY.** Unprompted, grade 26 built at round
33's exact `lede1` prescription (`sy 0.9815, ty 0.9490`) and its rig returned band
5.1769 -> **5.5955**, verbatim what this file recorded for that build seven rounds
ago. An instrument that reproduces the present is worth something; one that
reproduces a number from seven rounds back is worth more.

### ROUND 40 (GRADE 25, **B+**): FIVE PREDICTIONS, FIVE EXACT LANDINGS, AND THE REFUTATIONS TAKEN IN THE RIGHT ORDER FOR THE FIRST TIME

Pixels **4.1645 -> 4.1483** against a predicted 4.1483; n_over8 **90968 ->
90833** against a predicted 90833. shield 9.3237 -> **8.5019**, diffMark 9.0344
-> **8.3575**, helpIcon2 6.8681 -> **6.3798**, resendLab 6.4055 -> **6.1780**,
resendVal 2.2172 -> **2.0483**, diffBtn 3.8481 -> **3.8069**. **38 of 44 bands
byte-identical.** Every figure is grade 25's, to four decimals. Gates 11/11, 3/3,
7/7.

**RULE 102 WAS OBEYED THIS TIME AND THE ORDER IS THE POINT.** All five refuting
measurements were taken on the built capture BEFORE `report005` was run, with an
estimator written this round that shares no code with grade 25's rig — and that
estimator reproduced the grade's SHIPPED-STATE readings digit for digit on three
of the five (shield left -0.010/-0.055/-0.100 and right -0.532/-0.784/-0.787;
diffMark top +0.742/+0.312/+0.186; helpMark2 left +0.272/+0.451/+0.281). Two
independently written instruments agreeing to three decimals on a quantity
neither of them was tuned against is the strongest cross-check this screen has
had.

        landmark                     shipped              round 40        required
        shield right crossing        -0.787               -0.180          |x| < 0.20
        shield left  (must HOLD)     -0.100               -0.075          |x| < 0.15
        diffMark top                 +0.742/+0.312/+0.186 +0.106/+0.092/+0.008
        helpMark2 left               +0.451               +0.052          |x| < 0.15
        resendLab cap-top            +1.267               +0.685          +0.69 +/- 0.15
        resendVal whole-run dx       +0.345               +0.110          |x| < 0.15
        help1 control                +0.180               +0.180          unmoved

**THE SHIELD'S LEFT EDGE HOLDING IS THE HALF THAT MATTERS.** Grades 18 and 19
proved the shield's CURVE is wrong and that no y-affine reaches it, and this file
has twice refused a shield move as paper-over. D1 is a different axis — its
WIDTH — and the refutation is what separates them: a translation papering over
the curve would have driven the left crossing to +0.25 and failed. It went to
-0.075. Width 97.4 -> 98.1 was predicted from the extent ratio alone (97.4 x
69.630/69.109 = 98.135) before any sweep, and the objective's joint (w, tx)
argmin contains the prediction.

**D2 IS THE THIRD TIME AN AXIS WAS CLOSED BY A REFUSAL THAT ONLY COVERED
`size`.** `resendLab`'s own note in the recipe records the defect verbatim — "no
cap estimator returns 1.0000 on the shipped artefact: 'R' 0.968, ascender 'd'
0.951, whole-run vertical extent 0.947 / 0.945" — and then closes the vertical
axis inside a sentence about the FACE. Round 30 had correctly separated this
run's POSITION error and fixed it with `dy +1.50`; the cap HEIGHT is the other
half of the same face difference and stayed under the same closure. Round 33 did
this to six runs, round 39 to help1, and this is the third instance. **A refusal
of a `size` change is not a closure of the vertical axis, and this file has now
paid for that confusion three times.**

**AND MY OWN BASELINE ESTIMATOR WAS UNSTABLE, WHICH IS RECORDED RATHER THAN
SMOOTHED.** D2's control is that the baseline must hold while the cap grows. My
last-fall crossing read the baseline moving 0.026 / 0.183 / 0.441 px across the
three coverage fractions — scattered, and at the widest fraction outside the
+/-0.15 the grade claimed. I do **not** treat that as confirming or refuting the
mechanism: a 50-row window on a single capital is contaminated by neighbouring
ink, and a spread of 0.4 across fractions is an instrument telling you it is out
of its depth. What carries D2 instead is grade 25's **rule-34 control**, which is
a build and not an estimate: a pure translation at `sy = 1` gives +0.0025,
+0.0025, 0.0000 (same raster) and +0.0486 at `ty` +/-0.5 and +/-1.0 device px, so
the gain is *unreachable* by moving the run. Rule 102's corollary in the
uncomfortable direction — the estimator I wrote is the one that failed here.

**D4 CARRIES A STATED COST AND IS PRESCRIBED ANYWAY, WITH THE ALTERNATIVE NOT
EXCLUDED.** `helpMark2` also has a ~1% width deficit, so the -0.1875 nudge trades
the right crossing (-0.200 -> -0.358) for the left (+0.451 -> +0.052). A (w x tx)
grid is **degenerate** in this window — `w 66.9 / tx -0.375` scores the identical
-0.0022 — so the two levers are not separated by the evidence available. Grade 25
graded its own finding MEDIUM for exactly this reason and said so; the worst
single edge improves and n_over8 falls 35, and the width alternative is on the
record rather than dismissed. **Note also that grade 24 tried this mark at +/-1
device px and reverted it** ("the extent became EXACT on all four edges and
helpIcon2 went 18.0968 -> 19.3035"). That refusal was correct AT 1 px. The true
offset is a fifth of a pixel — rule 90 restated at a new granularity.

### WHAT GRADE 25 CLOSED, INCLUDING THE LEAD I GAVE IT

**THE BRIEF'S STRONGEST LEAD IS CLOSED.** I pointed it at the horizontal
registration of the nine runs grade 24 never measured, naming `plateLab`,
`safe1`, `safe2`, `lede2` and `resendLab` as holding 17,530 px between them.
Swept at +/-0.25 and +/-0.5 device px through the validated instrument on **all
fifteen body runs: thirteen of fifteen are at argmin.** Only `didnt` (refused for
the fifth time, rule 71, n_over8 +23) and `resendVal` (D5) were not. **Two briefs
running, two leads refuted by the grader they were given to** — round 39's was
`diffBtn`. A brief's lead is a hypothesis, and pointing a grader at the wrong
place costs nothing as long as the grader is allowed to say so.

**`letter-spacing` IS A LEVER THIS RECIPE DECLARES, EMITS, AND NO GRADE HAD EVER
SWEPT.** Sixty builds at +/-0.05 and +/-0.10 device px of per-gap advance across
all fifteen runs: **argmin everywhere.** The only mean gains are `didnt +0.05`
(n_over8 +48) and `wordmark -0.05` (n_over8 +6), both rule 71. That is a whole
declared axis closed for the first time in forty rounds.

**AND A TRAP WORTH THE WHOLE ROUND, on the shield and gear `ty` moves this file
has now refused three times.** The objective wants -0.75 on both. The ink
CENTROID (-0.27, -0.79) and a whole-profile SSD offset (-0.255, -0.590) **both**
say "translate up" and **both are wrong** — a bottom-anchored height deficit
displaces a centroid exactly like a translation does. Only the paired crossings
separate them: shield top -0.210/-0.114 against bottom +0.462/+0.362, gear top
+1.332/+1.460 against bottom -0.142/-0.346. **Edges moving oppositely is a size
error.** Two estimators agreeing is not two independent estimators — they can
share a blind spot, and here they share the one that matters. See rule 103.

Also refused with numbers: `gear` box height (the argmin pushes the envelope 1.5
px outside canonical's, which `Marks005` already records); `shield` box height (at
the honest geometry-landing point it is worth -0.001 to -0.002, and the metric's
-0.0078 is only reachable at a value that overshoots the bottom edge by 0.64 px);
`helpMark1 ty` (n_over8 -37 at zero mean, refused under D1's own standard because
it takes an ALIGNED top crossing from -0.003 to +0.316); `helpMark3` box height
(its extent is inside 1% and shrinking it drives the worst edge from +0.365 to
+0.79); `back` tx and ty (rule 71); `chev2` (below the level at which a shipped
constant gets moved).

### ROUND 39 (GRADE 24, **A−** — the first above B+ in twenty-four): THREE PREDICTIONS, THREE EXACT LANDINGS, AND THE ONE THAT DISAGREED WAS MY INSTRUMENT

Pixels **4.2079 -> 4.1645** against a predicted 4.1645; n_over8 **91263 ->
90968** against a predicted 90968; help2 9.1930 -> **8.1496** (predicted 8.1496),
help3 5.7610 -> **5.3430** (predicted 5.3430), help1 7.1172 -> **6.8700**
(predicted 6.8700). **41 of 44 bands byte-identical.** Gates 11/11, 3/3, 7/7.

**THE INSTRUMENT WAS THE WHOLE CANVAS THIS TIME, NOT THE ELEVEN MARKS.** Grade 23
rebuilt the marks and hit 0.0000 in every icon window; grade 24 extended that to
the entire 853x1844 and hit **0.0000 mean|d|, n_over8 0** against the shipped
render — a rule-40 control on the complete page, scored through `report005`'s own
windows. So its ~330 candidates were not estimates with error bars. They were
builds. That is why three separate predictions landed to four decimals and why
the three compose exactly (-0.0057 -0.0263 -0.0114 = -0.0434, realised -0.0434).

**D3 IS THE FINDING, AND ITS CAUSE IS A REFUSAL READ WIDER THAN IT WAS WRITTEN.**
Round 33's table is titled "THE VERTICAL SCALE OF EVERY BODY RUN WAS NEVER
SOLVED" and it lists eight runs. **The three help rows are not among them.** The
reason sits in the help-row comment: round 3 tried `size 13.9 / scale 0.809`,
measured it worse, reverted it, and closed with "the LENGTH is the axis this band
is scored on and the cap shortfall is the cheaper of the two errors" — which
retired the entire vertical axis on the strength of an experiment that only ever
touched `size`. **`sy` reaches the cap without touching the advance**, so the
refusal never covered it. Rule 92(a) sitting on top of rule 92(c), which is the
identical shape round 33 was itself written from, one axis over.

help1's cap ratio was **0.9711 / 0.9719 / 0.9719 with the baseline already
landed**, which at this emitter's pivot is an `sy` and cannot be a displacement.
`sy 1.02` with `ty 1.8429 -> 1.4680` took it to **0.9994 / 0.9974 / 0.9911 while
the baseline moved 0.01 px** (1342.66/1342.39/1342.12 -> 1342.66/1342.40/1342.13).
Both halves, which is the whole test. Rule 100 was **swept rather than assumed**:
the raster is constant over a 0.9 px plateau and 1.4680 is its CENTRE, so the
change survives a +/-0.45 px arithmetic error in either direction.

**D1 AND D2 ARE THE SAME DEFECT AT DIFFERENT SIZES, AND RULE 74b'S REFUSAL HAD
EXPIRED.** help2 was 0.35 device px left and help3 0.15, both with k = 1.0007 and
1.0009 — width landed, registration not. Eleven rounds ago rule 74b refused a
help2 translation as "five px NARROWER, not displaced", and that was TRUE THEN;
round 30's `scale` and round 36's `ws` closed the width, and rule 91's masking is
what was underneath. The envelope still disagrees (dL +0.87, dR -0.98) and the
envelope is the noisy read — canonical's ink runs further on BOTH sides, its
unsharp ramp, rule 90 — while a 5x5 (scale, tx) grid brackets the shipped 0.8561
on both axes.

### AND THE REFUTING MEASUREMENT DISAGREED WITH THE GRADE, AND THE DISAGREEMENT WAS MINE

Rule 101 asks for a refutation the builder can take independently. I wrote my own
estimator rather than reuse grade 24's, which is the point of one — and it came
back reading help3's shipped registration at **+0.106 against the grade's
+0.391**, i.e. it did not corroborate D2's diagnosis at all, while the band had
just landed the grade's prediction to four decimals.

The disagreement was **my segmentation**. I was merging adjacent glyphs at
`gap=2`, getting n=9 where the grade got n=23, and a merged cluster's
registration is dominated by its widest member. Re-segmented comparably it reads
**+0.370 against +0.391**. And the estimator that cannot be blamed on
segmentation at all — ONE sub-pixel dx fitted over the entire ink profile —
settles it in both runs:

        whole-run registration      shipped    round 39
            help2                    +0.430      +0.250
            help3                    +0.320      +0.210
            help1 (control)          -0.022      -0.020

**THE LESSON IS THE ORDER, NOT THE ARITHMETIC.** I read the bands before I took
the refutation, so when the two disagreed I already knew which one the build
agreed with — and that is precisely the position rule 101 exists to keep a
builder out of. Had the refutation come first I would have spent the next hour
deciding whether to trust a grade against my own measurement, which is the
expensive and correct version of this. **A refuting measurement taken after the
band is an attribution check, not a refutation**, and it should be recorded under
the weaker name. See rule 102.

**A RESIDUAL IS STATED RATHER THAN FORCED.** Both runs are still registered
+0.21 to +0.25 device px left after the fix, and more `tx` does not reach it: the
objective turns before the registration does (help2 +0.40 -> -0.0248 and +0.50 ->
-0.0236 against +0.35's -0.0263). The argmin of the metric and the zero of the
landmark are 0.25 px apart, and this file does not close that gap by breaking the
metric.

**WHAT GRADE 24 CLOSED IS LARGER THAN WHAT IT OPENED.** ~330 built candidates
returned the shipped value as the argmin across every lever the recipe owns:
vertical `ty` at +/-1 device px on all fourteen body runs (best alternative
+0.0024 — **the vertical axis is closed**); `scaleX` at +/-0.3% and +/-0.6% on
thirteen; `word-spacing` at +/-0.5 and +/-1.0 device px on eleven, **including
the four round 36 left "STATED AND NOT PRESCRIBED"**, all worse, so those are now
measured rather than asserted; `weight` +/-20/+/-40 on fourteen; the paper,
orange and box-rule tokens; all eleven `MARK_BOXES` at +/-1 px in both axes and
every mark `sw` at +/-0.25; the plate, the code boxes, all four dividers, the
header rule, the link rule and the caret. Rule 50's uncovered region — 508 rows
and 570,114 px outside the windows — holds **0 pixels over 8**.

**AND IT REFUTED THE BRIEF'S OWN STRONGEST LEAD.** I pointed grade 24 at `diffBtn`
because it carries 9132 px, 10% of n_over8, at a band mean of only 3.8481 — the
signature of a small systematic offset over a large object. It is not: grade 24
modelled Skia's supersampling blitter (4 sub-scanlines per row at `row+0.125+0.25k`
in y, analytic in x), **validated the model against the shipped raster first**
(predicting the top edge 127/0/63, bottom 190/0/63, walls 92/0/148 and 69/0/172,
max error 0.85 of 255), and a joint search over `sw x dy x dx x dh` returns the
shipped (2.05, 0, 0) at **gain 0.0000**. The residual is canonical's own ringing —
rows 1042-1046 and 1053-1057 read 244-250 against flat 254 paper on BOTH sides of
the border with a clipped 255 overshoot between, a decaying resampling lobe no SVG
can draw. Edge centroids agree to 0.09, 0.06, 0.03 and 0.05 px. **The +/-1.07
inset and the 2.05 stroke are right, and a brief's lead is a hypothesis like any
other.**

**TWO GAINS WERE REFUSED AGAIN ON THE SAME GROUNDS.** Graphite `#4B4D57` buys
0.0029 with **n_over8 +17** (rule 71, re-measured and re-refused); `didnt`'s
`tx +0.5` buys 0.0089 with **n_over8 +23**, and its per-glyph registration reads
+0.474 with **sd 1.129** — the scatter exceeds the mean, two estimators disagree,
and three grades have now refused it. `shield ty-1` (-0.0060) and `gear ty-1`
(-0.0039) are paper-over: at threshold 200 the shield's dome top is aligned to
-0.03 and only the tip is off, so moving up 1 px takes an ALIGNED landmark to
-1.03.

**AND A REAL GEOMETRIC RESIDUAL WITH NO PAYING LEVER, recorded so the next round
does not rediscover it as free money:** help2's cap is **3.7%** short and help3's
**7.2%** — both LARGER than the help1 shortfall this round closed, both past rule
97's cutoff — and a 2-D (sy, ty) grid with each run's own `tx` argmin held is
worse at every point (help2 sy 1.02 +0.0095, 1.04 +0.0301, 1.06 +0.0387; help3 sy
1.04 +0.0147, 1.08 +0.0114, 1.12 +0.0259). The cost of D3 is stated with it:
help1 now stands 2% taller than its two neighbours, and the column reads 21.2 /
20.2 / 20.0 against canonical's 21.3 / 20.9 / 21.6 — one row landed instead of
none.

### ROUND 38 (GRADE 23, **B+** — the best in nine): PREDICTED TO FOUR DECIMALS, LANDED TO FOUR DECIMALS

Pixels **4.2180 -> 4.2079** against a predicted **4.2079**; n_over8 91397 ->
**91263** against a predicted 91263; `plateMark` 7.2395 -> **4.5255** (predicted
4.5255) and `plate` 2.7670 -> **2.6238** (predicted 2.6238). 42 of 44 bands
unmoved. Gates 11/11, 3/3, 7/7; desktop clean; rule-40 control exact.

**THE PREDICTIONS WERE EXACT BECAUSE THE INSTRUMENT WAS.** Grade 23 rebuilt all
eleven drawn marks as a standalone page at the shipping DSF and measured **0.0000
mean|d| against the shipped render in every icon window**. Its numbers are the
shipping rasteriser, not a resample, so nothing here is an upper bound. That is
the first time on this screen a prediction and a build have agreed to four
decimals on a size change.

**RULE 101 WAS OBEYED AND IT PAID.** Grade 23 supplied the refuting measurement
unprompted: plateMark's bottom horizontal edge must read **3.67 +/- 0.12** device
px (3.000 before) and its left wall **3.95 +/- 0.15** (3.166). Checked BEFORE any
band: **3.670 and 3.942.** The width landed, so the band movement is attributable.

**THE DEFECT HAD BEEN CLOSED ONCE, ON THE AXIS WHERE IT IS SMALLEST.** The
recorded refusal measured the WALLS, where a linear-light alpha reads canonical
2.839/3.042 against render 2.737/2.995 — the ~+0.1 agreement this file records.
The defect is on the HORIZONTAL edges, which the geometric mean makes the thinnest
thing in the mark: canonical **3.930 against 3.000**, a whole device pixel. Rule
57 in the other axis. And the compositing premise that licensed the refusal is
refuted by canonical's own plate: linear-light compositing would put an
sRGB-normalised 50% crossing at true area 0.214 and every orange/white edge 0.57
px off, while canonical's plate measures **741.812 against the render's 741.760**
on the identical plane and estimator — 0.052 px, a **ten-fold margin**. The two
plates composite alike and the 0.77x ink-mass reading was right all along.

### AND RULE 101'S EXAMPLE WAS WRONG — ROUND 37 MEASURED A NULL EXPERIMENT

`vector-effect` **is not an inherited property.** `Icon` carries `strokeWidth` on
the `<g>`, so that is where round 37 put `vectorEffect` — and the paths kept
`vector-effect: none` and drew the plain scaled stroke. Four variants at
helpMark1's exact box, widths by integrated mass with `getComputedStyle` read back
on each:

    A  sw/sqrt(sx.sy) — shipped     v 3.3451  h 3.0000  h/v 0.897   none
    B  nss ON THE PATH, sw          v 6.8353  h 7.0039  h/v 1.025   non-scaling-stroke
    C  nss on the path, sw/sx       v 2.4471  h 2.5020  h/v 1.022   non-scaling-stroke
    D  nss ON THE <g>, sw           v 8.7882  h 7.7529  h/v 0.882   NONE

Round 37 measured 8.879 and 9.914. **Variant D predicts 8.788 and 9.728** — the
geometric mean removed and nothing put in its place. Variant B is isotropic and
lands at `sw x S` (3.15 x 2.1705 = 6.837 against 6.835), so **the grade that
guessed the screen CTM had the constant right as well as the class.** Rule 101's
lesson stands and is now sharper: rule 99 proved the injection took ON A PROBE,
and the build wrote the property somewhere it cannot inherit from. *Proving a
declaration applied to element X is not proving it applied to the element that
paints.*

**THE LEVER THEREFORE EXISTS AND IS STILL NOT WORTH BUILDING ALONE.** Isotropy at
the shipped `sw` values is measurably WORSE (+0.192 helpIcon1, +0.234 helpIcon2,
+0.223 plateMark, +0.165 shield against −0.071 gear, −0.078 diffMark). The full
nine-mark joint solve is worth **≈0.016 of whole screen**, of which plateMark
alone — built here without it — is 70%, and it takes n_over8 lower than the nss
version does (91263 against 91279). Two of the remaining argmins overshoot
canonical's measured widths (helpIcon1 would draw 3.19 walls against a measured
3.05), which is the paper-over this file declines on `display` and on the digits.
**Not prescribed, with the numbers.**

A STALE CONTROL WAS ALSO CAUGHT, in `phone-005.ts`: the zoom rule's note gated
itself on "the 393 render must come back at md5 `661b4275…`" — the ROUND-32
render, stale four rounds later, and it would have fired falsely for whoever ran
it next. It now names the CURRENT ARTEFACT block and `artefact-check.mjs`. **A
control that names a moving value has to name where the value lives.**

RE-DERIVED CLEAN AND NOT TO BE RE-OPENED: `display` is face-limited down to the
mechanism — canonical's modal stem is 11.854 px against the render's 11.098, which
the recipe predicts exactly, while canonical's horizontal BARS are 11.2 against
the render's 16.7, **49% over**, because `scaleX(0.5921)` shrinks stems and leaves
bars alone. That also retires `-webkit-text-stroke` here with a reason rather than
a sweep (it adds 1.285t to a stem and 2.17t to a bar, fattening the already-fat
axis 1.69x harder), and re-derives the cut independently: needed stem/advance
0.381 against medium 0.278, **semibold 0.360**, bold 0.456. 36% of the screen's
total error sits in `display` and none of it is reachable. The vertical text axis
is closed too: GeistVF's x-height/cap is 1.065 +/- 0.007x canonical's over eight
runs, so no `sy` lands both landmarks, and every body run's vertical placement is
within 0.37 px of the excess-splitting optimum — inside rule 100's plateau.

### ROUND 37 (GRADE 22, **B** — the first above B- in seven): A MECHANISM REFUTED BY ITS OWN FIRST CONTROL

Pixels **4.2260 -> 4.2180**, n_over8 91510 -> **91397**. Gates 11/11, 3/3, 7/7;
desktop clean; rule-40 control exact; 36 of 44 bands unmoved.

    helpIcon1 6.8633 -> 6.2339   helpIcon2 7.4148 -> 6.8681   back 3.5029 -> 3.2205
    helpIcon3 7.5455 -> 7.2985   gear 8.6972 -> 8.5389      shield 9.3281 -> 9.3237
    diffMark  9.0047 -> 9.0344  (+0.0297, kept)

**THE MECHANISM FAILED AT THE ASSERTION THE GRADE WROTE FOR IT**, and that is the
round's most useful sentence. Grade 22 refused to treat D4's verified premise as
sufficient and specified the missing check: *"the first assertion of the build is
a re-measure of one vertical wall — helpMark1 at col 60 must read 3.15 +/- 0.05,
not 6.8 — before any band is read."* Built, it read **8.879**, and diffMark's read
**9.914** against a prescribed 3.04.

The realised factor is `sx`, not 1: 8.879/3.15 = 2.819 against sx 2.7917, and
9.914/3.04 = 3.261 against sx 3.2000. This Chromium resolves `non-scaling-stroke`
against a coordinate system that still carries the horizontal scale. The grade
guessed the screen CTM (2.1705) — so its CLASS was right and its constant was
not, and the control it insisted on caught it before one band was read. This is
the sharpest available example of rule 99's real scope: **the premise of D4 was
verified twice — the injection took, the viewBox arithmetic was read off the
source — and the REALISED EFFECT was still something else.** Verifying that a
lever is wired is not verifying what it does.

**THE ANISOTROPY IS STILL OPEN** and stated: horizontal strokes remain ~11%
thinner than verticals on every anisotropic mark. Closing it needs a lever that
is not this one — per-axis path outlines, or isotropic mark boxes, which rule 90
has refused three times.

**WHAT SURVIVED IS THE HALF THAT NEVER NEEDED THE MECHANISM.** Eight marks carried
one shared `sw = 3.0` from a single sentence, while canonical's eleven strokes
span **2.65 to 3.70**. Every `sw` is now canonical's own measured width — from
50%-crossings over n=12-47 lines per mark, cross-checked against net signed ink
mass and a joint (centre, width) L1 objective. Two controls make it trustworthy:
the `diffBtn` border reads canonical 2.023/2.083 against a solved 2.05, so
**canonical is not inflated by its unsharp mask at these levels**; and the three
chevrons read mass R/C 1.004 / 0.949 / 0.990 — a mark that needs nothing comes
back needing nothing.

`back` is the clearest case and the grade called it in advance: its box is
**exactly isotropic at 50.5 x 50.5**, so `vector-effect` was never going to change
one pixel of it, and its stroke was still 0.35 px thin. The re-solve is the whole
of its fix, and it is the largest single gain in the round.

### AND GRADE 22 NAMED THE CAUSE OF THE TWO LARGEST UNEXPLAINED WINDOWS

`digit0` 9.461 and `digit1` 10.470 were listed as having no known cause. They have
one: **canonical's Tungsten is a heavier CUT than Medium.** At a landed cap
(C/R 0.9978 / 0.9992 / 0.9956 / 0.9943) and a landed advance, canonical's stems
are pooled **13% wider** (C 7.175 against R 6.348, n 228/232) and its counters
**14% narrower** (−1.49 px on the "8", −1.32 on the "2"). The counters closing by
about twice the stem excess is the discriminator: the stems have grown INWARD at a
fixed advance, which is what a heavier cut does and what a text-stroke does not.

Rule 44 settles it. Rasterised with FreeType at the shipped em, **the render is
Tungsten Medium to 0.03 px** — which is what licenses the rest — and canonical
sits between Medium and Semibold at fraction 0.39, an interpolated weight of about
**478**. Neither bundled cut is it: Medium is 14% light, Semibold 19% heavy. The
`wordmark` is the control that this is not a global canonical darkening — Geist
stems read C/R 0.9567, i.e. canonical is LIGHTER there.

**This is a fifth NEEDS KEVIN item, and grade 22 deliberately did not prescribe
it.** `font-weight` cannot express 478 (CSS matching sends 470 and 500 alike to
the 400 face), so the reachable set is {Medium, Semibold} and Medium is already
the nearer. The one unused lever is `-webkit-text-stroke-width`, declared in `Run`
and emitted by `runCss` and used by no run here — and a synthetic-stroke sweep
puts the per-glyph argmin at 0.4-0.8 px for three digits and at **0** for the "2",
while buying the stem at the price of a cap landed to 0.6%. That is the trade this
file declines every time.

TWO MORE, RECORDED. The code-box border tone is a **confirmed null**, re-opened
because the 7x7 sweep that chose 1.75/#767676 predates `BOX_Y` and `BOX_W` landing
in the same pixels: the ink deficit is real (canonical 268.6 units against 238.6)
and an analytic fit says the width is right and the tone ~12 levels light, but a
full-window synthesis whose control reproduces the shipped bands EXACTLY puts the
argmin at 0.0008 of whole screen with the five per-box argmins disagreeing. And
`LINK_RULE`'s note says its centre lands on 845.565 when the shipped values give
**845.535** — the `y` was derived at `h 1.97` and not re-derived when the fit moved
`h` to 1.910. 0.031 px, unreachable; the sentence is what needs correcting.

### ROUND 36 (GRADE 21, **B-**): THE LETTERS ARE 2% WIDE AND THE WORD SPACES 1.5 px NARROW, AND THE TWO CANCEL IN THE ADVANCE

Pixels **4.3328 -> 4.2260**, n_over8 92030 -> **91510**, against a predicted
0.097-0.118 — inside the range. Gates 11/11, 3/3, 7/7; desktop clean; rule-40
control exact. 37 of 44 bands unmoved and **all ten named controls
byte-identical**.

    diffLab 16.5906 -> 12.6948 (pred 11.9660)   help2 10.3449 -> 9.1930 (8.9283)
    help3    6.6128 ->  5.7610 ( 5.4788)        help1  7.7874 -> 7.1172 (7.1726)
    diffMark 10.8033 -> 9.0047 ( 8.8500)        diffBtn 4.4026 -> 3.8463 (3.8700)

**THE MECHANISM SURVIVED THIRTY-FIVE ROUNDS BECAUSE IT IS INVISIBLE TO THE
ESTIMATOR THOSE ROUNDS USED.** `scaleX` is solved against a run's total ADVANCE. A
face difference split between glyph advances and the SPACE advance cannot be seen
that way — the letters absorb the whole error, the spaces absorb the opposite one,
and the extent lands anyway. `word-spacing` is declared in `Run`, emitted by
`runCss`, and used on exactly one run here (`display`, where round 4 found it for
this same reason) and was never tried on a body run. Rule 92(b) on a lever this
file already owns.

Three estimators that share nothing: per-glyph ink widths put the GLYPHS wide
(1.0198 / 1.0338 / 1.0316 / 1.0134), so the lever is `scale` and not
`letter-spacing`; a matched-glyph centroid regression with a word-gap term
separates the two (b +2.235% / delta -1.738 px on diffLab, halving the rms against
translate-only); and an image-space grid brackets on BOTH axes at the regression's
own values. The raw residual shows it directly — inside "different" it ramps -1.35
to +0.47 across one word and drops 1.2-1.4 px at every word boundary — and 33 of
34 ink-edge word gaps across ten runs are narrower in the render.

THREE DIFFERENT PIVOTS were in play and each is stated in the recipe: `scaleX`
pivots on the element left (a 2.2% change moves the ink-left by <= 0.018 px, so
`left` needs no compensation); `word-spacing` is a LAYOUT property applied BEFORE
the transform, so its device effect is `ws x scale x S`; and `tx` sits inside the
transform after `scaleX`, so its device value is held by `tx x scale_old/scale_new`.

**AND `help1` BEAT ITS PREDICTION DESPITE BEING BELOW RULE 97'S CUTOFF — WHICH
BOUNDS THE RULE.** Sorted by magnitude the batch was 2.235 / 2.177 / 2.031 /
**0.671**, and rule 97 says to expect the split at the smallest: lede1 failed at
1.89% and resendVal at 2.3%. help1 is 0.671% and landed 7.1172 against a predicted
7.1726. The grade hedged exactly this in advance — the `ws` half is a LAYOUT
change, not a re-rasterisation at a new size, so the fixed hinting cost that
produces the 2.4% boundary does not exist for it. **Rule 97 is therefore a
statement about SIZE changes, not about small changes**, and its text is amended.

**D2 — the pencil was stretched 11% in x by its own mark box.** The box is 76.8 x
68.5, so `Icon` applies `scale(3.2000, 2.8542)`; round 32 re-cut the ENVELOPE to
compensate and the pencil, whose defining feature is a 45-degree axis, never was.
Second moments over an 8-connected component: `k_y` **0.9998 EXACT** against `k_x`
1.1347, with thresholded extents agreeing independently (canonical 28x28 — square
— against the render's 28x31). Wrapped in `scale(0.9 1)` about the eraser end,
which already lands in both plates. 10.8033 -> 9.0047.

**D3 — the clock ring is an ellipse, and the cause is an arithmetic error in the
note that derived it.** `k = 15.58/7 = 2.2257` assumes the drawn outer radius is
`r + sw/2 = 7` units, but `Icon` divides `sw` by `sqrt(sx.sy) = 2.6675`, so it is
6.0623. `k` is neither `sx` (2.7625) nor `sy` (2.5750) and every number derived
from it inherits the error. **It cost 0.0074 of band and is KEPT with the cost
stated**, on the same footing as round 14's resendLink colour: DoD item 4 is
geometry measured, not a band mean. A second, cruder 33-ray estimator confirms the
SIGN (aspect 0.9655 -> 1.0189) while disagreeing on canonical's absolute aspect,
and is recorded as confirming the sign only.

**D4 IS MEASURED AND DELIBERATELY NOT BUILT.** Every anisotropic mark draws its
horizontal strokes ~11% thinner than its verticals, because `sw / sqrt(sx*sy)` is
a geometric mean and one scalar cannot express two axes — measured on unclipped
coverage profiles where the within-image h/v ratio is self-normalising, with
`helpMark3`'s near-isotropic box as the control where canonical and render agree
(0.997 against 1.020) while helpMark1 reads 1.041 against 0.873. The fix is
`vector-effect: non-scaling-stroke`. Held for its own round because it changes the
SHARED helper and all nine marks at once, because `sw = 3.0` was itself fitted
against an anisotropic stroke and would need re-sweeping after (rule 92's fourth
clause), and because the grade said plainly it had not injected `vector-effect`
and proved it took (rule 99). Building it beside three other findings would make a
regression unattributable.

### ROUND 35 (GRADE 20, **B-**): THE FIVE SIMPLEST OBJECTS ON THE CANVAS SHARED ONE HEIGHT AND ONE TONE

Pixels **4.3613 -> 4.3328**, n_over8 92453 -> **92030**. Gates markup 11/11, csrf
3/3, desktop-leak 7/7; desktop clean; rule-40 control reproduced 4.3613 / 92453
first. 34 of 44 bands unmoved.

    caret   2.1319 -> 1.4548     rule3  0.9089 -> 0.4385   n_over8 172 -> 0
    box4    2.0546 -> 1.6984     rule4  0.6528 -> 0.3042            56 -> 0
    rule1   0.8536 -> 0.5674     hdrRule 0.6514 -> 0.4404
    rule2   0.5189 -> 0.3751     resendLab 6.5762 -> 6.4055
    diffBtn 4.4780 -> 4.4026     resendLink 2.7422 -> 2.6799

**D1 IS AN INSTRUMENT FINDING AND IT RETIRES AN AXIS.** Skia takes rounded rects
through the supersampling blitter — four sub-scanlines in y, analytic in x — so a
horizontal edge is floored onto a **0.25 device-px lattice** while a vertical edge
gets exact coverage. Ten rows on three unrelated features fit that model to <= 1
unit of 255 where an analytic model is wrong by up to 0.11 coverage, and the
CONTROL is the plain rects (hdrRule, dividers, caret), analytic-exact in both
axes. Consequence: `BOX_Y`, `BOX_H`, `PLATE.y/h`, `DIFFBTN.y/h` and the viewBox's
`-0.4` cannot be tuned below 0.25 px on their horizontal edges. Round 29's
`BOX_Y` 0.35 landed because it crossed two sub-scanlines, not because it was 0.35.

**D2, THE LARGEST FINDING, AND THIS FILE HAD ALREADY MEASURED IT.** The
header-rule note lists canonical's five 50%-crossing heights as "1.645 / 1.679 /
1.783 / 1.312 / 1.911, mean 1.67" — a 46% spread — and then solves ONE joint
argmin over all five; the divider note lists canonical's four inks and says "the
render's flat 72.0 sits at their mean, which is why they are left alone".
**Sitting at the mean is an L2 argument and the metric is L1**, for which the
per-element fit strictly dominates. Rule 92(a) on top of rule 92(b). And one part
is not a trade at all: canonical's rule3 core reads 204.6 over 730 columns and a
`#D4D4D4` fill is 212 at FULL coverage, so no height whatever reaches it — the
tone HAS to be per-rule. Divider n_over8 228 -> 0.

**D4 — THE INK-MASS ESTIMATOR HAS NO FIXED POINT**, because its answer is a
function of its window: canonical's underline ink is 483.9 units over rows
843..847 and 511.4 over 838..852, the extra 27.5 being the unsharp DARK halo a
render on flat paper does not have. The render's mass is proportional to `h`
alone, so widening the window can only inflate the answer. It had shipped three
constants too large — `CARET.w` 3.08 -> 2.92, `LINK_RULE.h` 1.97 -> 1.910, focused
box stroke 2.06 -> 1.83 — and the UNFOCUSED 1.75, read from eroded cores rather
than ink mass, is unchanged and is the control that localises it to the estimator.

**D3 — diffBtn's border is half correctable and the other half is refused in
advance.** Rows 1046 and 1051 read 244.4 and 236.5 against flat 254 paper seven
rows clear of the border, with 1150-1155 mirroring them: canonical's unsharp
under-ring, ~1400 pixels over-8 AGAINST BLANK PAPER that no SVG can draw. That is
what the band is made of, which is why n_over8 moves only 9342 -> 9296.

### THE TWO REFUTATIONS, AND THE FIRST IS A NEW PHYSICAL LIMIT

**RULE 100. THE VERTICAL TEXT POSITION IS QUANTISED TO 1.0 DEVICE PX.** Built
exactly as prescribed, `help1` (-0.38 device px) and `lede1` (-0.40) produced
**ZERO changed pixels** — byte-identical rasters — while the served stylesheet
carried the new translate. Swept live with the injection read back (rule 99),
help1's raster takes exactly three distinct values across +/-1.2 device px:

    dDev -1.2 .. -0.4   raster A
    dDev -0.3 .. +0.6   raster B   <- shipped
    dDev +0.7 .. +1.2   raster C

One plateau ~0.9 px wide with the shipped position inside it. **The objective's
own argmin for these runs (-0.320, -0.340) is UNREACHABLE** — every `ty` in that
interval renders the shipped pixels exactly, and the nearest reachable move is a
full pixel, two to three times the measured error. Both reverted: the measurement
stands, the lever does not reach it. `resendLab`'s +0.48 crossed a boundary (1673
changed pixels) which is why one of three paid and two could not.

It also explains a shape in the record rather than only this round: every `ty`
prediction this screen has landed to four decimals was a full pixel or more, and
every sub-pixel one has been a no-op or a surprise.

**AND RULE 97'S CUTOFF IS CONFIRMED WHERE THE GRADE SAID TO LOOK FOR IT.**
`resendVal`'s `sy 0.9771` — a real 2.3% cap error agreed by two estimators —
measured **2.2172 -> 2.4232, worse by 0.206**. Reverted. lede1 failed at 1.89%,
this at 2.3%, and round 33's five that paid were 2.4% to 14%. The grade
prescribed it *predicting* that a refutation would be evidence about the CUTOFF
rather than the diagnosis, and that is what it is.

D8 is a re-measured null worth keeping: help1/2/3 caps are 0.977 / 0.962 / 0.928
and resendLab's is 1.9% short, but for all four the objective wants the OPPOSITE
sign, because Geist's x-height/cap is larger than canonical's and the x-height
carries the ink. Re-measured on a much cleaner artefact, the trade this file
already states twice is unchanged.

### ROUND 33 (GRADE 19, **B-**): A WHOLE DEGREE OF FREEDOM WAS NEVER SOLVED, AND THIS FILE DECLARES THE LEVER ITSELF

Pixels **4.5067 -> 4.3613**, n_over8 93525 -> **92453**. Gates SCREEN=005 markup
11/11, csrf 3/3, desktop-leak 7/7; desktop re-probed clean; rule-40 control
reproduced 4.5067 / 93525 first.

    safe2      8.8160 ->  6.6762  (pred  6.3334)   diffLab 18.1628 -> 16.5906 (16.8102)
    lede2      8.0204 ->  6.5651  (pred  6.6265)   plateLab 10.8933 -> 9.5243 ( 9.5649)
    safe1      9.7149 ->  8.7981  (pred  8.9175)   resendLink 3.3690 -> 2.7422 ( 2.7498)
    gear       8.8566 ->  8.6972
    carried    diffBtn 4.6583 -> 4.4780     plate 2.9710 -> 2.7670

Three of six beat their predictions, `resendLink` hit its to four decimals, and
every one of the seven improved on **n_over8 as well as** the mean.

**`Run.sy` HAD NEVER BEEN USED ON THIS SCREEN.** It is declared in `phone-005.ts`
as *"scaleY — cap height WITHOUT touching the advance"*, emitted by `runCss`, and
shipped on 004 as `display.sy: 1.014`. The recipe fits each run's `size` against a
cap and its `scale` against an advance and then never re-measures the cap. Caps
read at 0.35/0.50/0.65 of each SEGMENT'S OWN peak coverage — not an absolute
level, because the graphite runs never reach 0.5 absolute, which is why an
absolute threshold mis-read them for thirty rounds:
...
**DoD item 2 — cap measured per ink run — was failed on five runs by 2 to 14%,
and TWO OF THEM ARE RECORDED IN THE FILE AS SOLVED.** `safe2`: "Round 2: cap
1.000, advance 1.000 — solved". `safe1`: "the cap matched EXACTLY on the first
glyph (27 device px in both images)" — an INTEGER read of 26.262 against 27.265,
which is rule 25's own warning sitting in the file as a closure.

**AND ROUND 31 NAMED THE WRONG PARAMETER — rule 94 applied to my own text.** It
measured safe2's vertical at 10% and wrote *"a size error would give equal
...
compensates size with scale on eight runs. The diagnosis was right; the parameter
was `sy`, not the face. That is rule 92(c) for the third time on this screen — a
blocked cause absorbing a live parameter.

THREE THINGS MADE IT EVIDENCE RATHER THAN A FIT, before any build. The null set is
clean: the two runs whose cap ratio is 1.00 are exactly the two the sweep leaves
alone (+0.0004, +0.043). **`dy` alone is worth 0.000** on safe2, safe1 and lede2 —
the baseline error was invisible under the size error, rule 91's masking, and the
reason every previous vertical search on these runs returned nothing. And the
sweep's joint `dy` equals minus the independently measured baseline delta on eight
runs at mean |diff| **0.08 px**, from two estimators that share nothing. Coherence
check: once each cap lands, `scale x cap_ratio` is 0.8365 / 0.8362 / 0.8397 for
safe2 / plateLab / safe1 — three independently solved runs converging on one
scaleX, which is what a uniform face-width difference looks like.

PIVOT, stated (rule 93): `runCss` emits `scaleX(scale) scaleY(sy) translate(tx,ty)`
with `transform-origin: 0 0`, so `sy` pivots on the ELEMENT'S TOP-LEFT — not the
baseline, not the cap-top — and `translate` comes after the scales, so `ty` is
multiplied by `sy`. A full 1 px error in where the element top lays out moves the
baseline by 0.11 device px, so this survives rule 53. THE OBJECTIVE'S ARGMIN WAS
DECLINED with its cost stated: the metric wants more shrink than the cap does on
every run (safe2 0.850 against 0.880) because the x-height carries most of the ink
and Geist's x-height/cap is not canonical's, and taking it leaves the cap 5.4%
short in the OTHER direction — the paper-over this file already declined on
`display`.

### `lede1` WAS REFUTED, AND IT IS THE ROUND'S MOST USEFUL RESULT

Built exactly as prescribed (`sy 0.9815`, `ty 0.9490`) it went **5.1769 -> 5.5955**
against a predicted 4.5903, with n_over8 3400 -> 3538 — worse on both metrics
while the other five improved on both. Reverted, and the revert restores 5.1769
to four decimals.

**The discriminator is the CAP ERROR, not the confidence rating.** The five that
paid had caps 2.4% to 14% out; lede1's is 1.89%, the smallest of the six, and the
only one that lost. A real `scaleY` RE-RASTERISES the glyphs at a new size rather
than resampling the image the way the sweep did, so it carries a hinting and
stem-rendering cost that is roughly FIXED while the geometry gain scales with the
error — **below about 2% the cost wins.** Both runs the grade flagged MEDIUM
(diffLab, lede2) beat their predictions, so confidence was never the signal;
magnitude was. See rule 97.

That also puts a floor under the rest of this axis: help1, help2, help3, resendVal
and the wordmark all sit inside 2%, so there is nothing further to take on `sy`,
and the two the grade explicitly declined stay declined for a second independent
reason (help1's ky 0.970 shrinks a cap already 2.3% SHORT; help2's cap-landing
measures worse by 1.06, reproducing this file's existing revert).

`gear`'s ellipse landed -0.159 against a predicted -0.205, and its value is the
CLASSIFICATION: a `circle` inside a non-uniform scale draws an ellipse, and
`Icon`'s arithmetic predicts the render exactly (r 3 + sw/2 = 3.5717 units x
sy 2.125 = 15.18 rows against canonical's 17). But the dial is 14.4% of that
window's difference and the COG is 86% — canonical's lobe band spans rows 37..68
against the render's 40..67 while the outer envelope matches to 0.045 px — and the
objective's gear argmin buys the lobes only by pushing the envelope 1.5 px OUTSIDE
canonical's. `gear` is an asset difference like `shield` and moves to NEEDS KEVIN.

FOUR NEGATIVES THE GRADE ESTABLISHED, so a later round does not re-spend them.
**The window set is complete**: the 34 windows leave 570,114 px uncovered carrying
mean|d| 0.347 and **n_over8 = 0**, so no defect hides outside the bands. **Colour
is right**: the plate shell reads canonical (253.3, 66.5, 0.79) against (253, 66,
1), and canonical's graphite dark tail (15-24 at its darkest against a hard render
floor of 73.7) is unsharp overshoot on a minority of core pixels — at the 5th
percentile canonical is 71.7-80.0 against that same 73.7 (rule 8). **The digits
are undisturbed by round 31** and reproduce round 28/30's recorded values exactly.
**Round 32's mark work lands**: a joint (kx,ky,dx,dy) sweep returns exactly 0.0000
for `plateMark` and `helpIcon1`, +0.09 for `diffMark`.

TWO MEASURED-AND-OPEN, both stated rather than prescribed. `diffBtn`'s border is
anisotropic in the render — one `strokeWidth={2.10}` draws 2.000/2.004 horizontal
against 2.102/2.106 vertical (sd 0.000 over 300 scan lines) — and canonical reads
2.354/2.217 against 2.079/2.093, but calibrating canonical's unsharp ringing on
the code-box border puts that inflation at 9-19%, larger than the effect. Rows
1046-1051 and 1155-1160 carry ~730 over-8 pixels each, 55% of the diffBtn band's
n_over8 outside its label and mark, so that is where the band lives; it is the
same compositing-artefact class that forbids a `sw` prescription on `plateMark`.
And `LINK_RULE`'s ink-mass estimator STILL has not converged: re-run on the
shipped artefact it returns 2.009 against the shipped 1.97 — it moves 0.04 px and
buys nothing, but the fixed point is not where the comment says it is.
...
since the screen was started.

    width  dpr   BEFORE                  AFTER
     393  2.17   clip 0                  clip 0, zoom 1, canvas 393x852
     375  2,3    clip 0, hScroll         clip 0, no hScroll
     360  2,3    clip 7, hScroll         clip 0, no hScroll
     320  2,3    clip 7, hScroll         clip 0, no hScroll

THE CANVAS IS SCALED, NOT REFLOWED — `@media (max-width: 392.98px){ .s5{ zoom:
calc(100vw / 393px) } }`. 393 is the design width and an invariant; reflowing
below it would abandon the canonical layout to fix a viewport problem. `zoom`
rather than `transform: scale` because zoom affects LAYOUT — hit-testing and every
descendant's box scale with it — where a transform leaves a 393px layout box
behind a scaled picture. And `calc(100vw / 393px)` divides a length by a LENGTH,
which is a number; the two shorter spellings tried first are both invalid and both
did nothing when injected live (see the diagnosis below).

**THE CONTROL IS BYTE IDENTITY AND IT HOLDS:** the 393 capture is
`f08b775035039f10a6a06a31ecc97b65`, identical to the committed round-33 render.
The query is gated at 392.98 and the capture is shot at exactly 393. Gates:
markup **11/11** with the reflow probe reading `ok/0clip` at all four widths, csrf
3/3, desktop-leak 7/7, desktop invariants clean.

STATED TRADE: below 393 everything scales, tap targets included — at 320 a 44pt
target becomes 35.8pt. That is what any scaled canvas does and it is strictly
better than seven controls being unreachable; the alternative is reflowing the
canonical layout, a larger decision than this defect warrants.

`markup-gate`'s `expectClipped` and `expectScroll` are now EMPTY rather than
deleted. They recorded the true state so a regression would fail, and the file
said so — but a tracked defect is not a fixed one. Empty, they still fail on any
control that starts leaving the viewport again.

**THE DESKTOP-LEAK GATE CAUGHT THIS CHANGE AND WAS TOO LITERAL**, which is the
round's second lesson. It went 7/7 -> 5/7 because a `.s5` rule now lives in a
SECOND media query. That is not a leak: a query whose max-width is NARROWER than
the phone breakpoint can never apply where the phone query does not, so it is
phone-scoped by construction. The test is now `max-width <= 767.98px` rather than
string equality, with a `min-width` or a wider `max-width` still counting as a
leak — those are the shapes that can actually reach desktop. Its self-test now
injects a rule with no media query at all, which is the shape it exists to catch.
Two gate corrections in two rounds, both in the same direction: **the first
version of a gate encodes the one case that prompted it, and the second correct
change is what teaches it the class.**

AND THE BACKTICK TRAP FIRED FOR THE THIRD TIME. The comment above the new query
was written with backticks around identifiers, inside `PHONE_CSS`, which is a
template literal; tsc gave TS1443 and the file was briefly unparseable. This
file's history records the same break twice before. Twenty-eight backticks
replaced with quotes. Three occurrences is not carelessness, it is a missing
guard — see rule 98.

### THE REFLOW RESIDUAL, AS DIAGNOSED (round 34 above closes it)
...
capture. **BUILT IN ROUND 34**, immediately after round 33 closed — a layout change while a
...
**ROUND 46 REFUTED IT, AND NOT ON THE TERMS THE PARAGRAPH SET.** The finding was
worth 0.0023, not 0.01 — so by the letter of the test above, the floor claim
survives. By its substance it does not. The paragraph's real claim was that the
METHOD had been rebuilt four times and the gains fell anyway, therefore the
remaining error was unreachable. That was wrong for a reason none of the four
rebuilds could have exposed: **every one of them rebuilt the same KIND of
instrument.** Byte-exact rasterisers, paired-crossing estimators, coupled
parameter grids — all of it geometry, because the band table is geometry-shaped
and each brief inherited the last one's framing.

The axis that broke the claim was `--s5-orange`, one character, correct value
already in the file's own docstring, found with the simplest estimator this
screen has used: an L1 median over an eroded plateau. **Convergence was measured
on the sampled axes and reported as convergence of the screen.** See rule 110.

So the honest state is: the per-round gain on GEOMETRY really has collapsed
(rounds 42-46 on geometric levers: 0.0069, 0.0051, 0.0009, 0.0014, and round 46's
geometric half was nothing at all), and the screen is not at its floor because
the axis inventory was incomplete. Those are compatible, and only the first was
measured.
...
| 7 | reachable by a real user path AND by the harness | **HOLDS — re-verified end to end** | /signup lands here (round 5); the harness step is deterministic and capture step-fails are 0 every round. And after round 36 all **14** visible phone controls were followed rather than asserted, because this item had been carried on one round-14 check for twenty-two rounds: `/signup` 200; `/settings` 307 to `/signin?from=%2Fsettings`, i.e. the auth redirect and not a broken route; **both guide anchors 200 with the id actually present** (`#email-spam`, `#email-delay` — one element each, the destinations round 14 BUILT); six code inputs, back and resend on JS handlers; and two `mailto:` links that a fetch cannot follow. The bare `mailto:` on "Open email app" is the documented fallback — `inboxUrl()` resolves 16 providers to their webmail inbox and falls back only for an unknown domain, which `example.com` is |
...
### NEEDS KEVIN: THE FIVE ASSET AND TYPE DECISIONS THAT NOW BLOCK 005 FROM AN A
...
> **STATUS AFTER GRADE 32 (final): THIS LIST IS NOW THE WHOLE OF WHAT REMAINS.**
> Forty-six measured rounds and thirty-two independent grades have taken the
> screen to **4.1139 / n_over8 90508**, best grade **A−**. Grade 32 completed the
> axis inventory rule 110 asks for — all 21 `Run` fields traced through `runCss`,
> all five CSS custom properties given an eroded-plateau audit — and **every
> declared axis is now swept, dead, degenerate, or closed for cause.** The last
> four grades prescribed 0.0010, 0.0014, 0.0023 and **nothing**, while refusing
> 0.0213, 0.0045, and every candidate they found.
>
> Each of the five below is now proved BY BUILD rather than argued:
>   1. all six affected runs built at their cap-landing `sy`, all six much worse;
>   2. the wordmark's ratio measured against all four bundled cuts;
>   3. three different letter widths required by one canonical row;
>   4. shield and gear refused four times each — the shield's fractional height
>      error is SIX numbers by azimuth (low at the shoulders, HIGH at the crown)
>      and the gear's cog is 28.2 px tall at every side azimuth where canonical
>      runs 30.0–32.1, so both are different CURVES and no transform reaches one;
>   5. canonical's Tungsten is heavier AND UNEVEN — per-glyph ink mass 1.0126 /
>      1.0655 / 1.0662 / 1.1593 — which is why the one continuous ink lever on
>      those runs (`-webkit-text-stroke-width`, worth −0.0159) had to be refused:
>      a uniform stroke drives digit0 from a landed 1.0126 to 0.9626.
>
> **Together they hold 36% of the screen's total error.** `display` alone is
> 19.3019 of the 4.1139 figure and 21% of `n_over8`. Grading has stopped; the
> screen is BLOCKED, not DONE, and the difference is DoD item 9.
...
**4. THE SHIELD AND THE GEAR ARE THE WRONG DRAWINGS, NOT THE WRONG SIZE, AND
CLOSING THEM MEANS RE-TRACING ASSETS.** The gear was added after round 33, on the
same evidence in a different shape: its outer envelope matches canonical to
**0.045 px** in height and 0.30 in width, and yet canonical's cog lobes span rows
37..68 against the render's 40..67 — the lobes reach further at the same envelope,
which no scale or position reaches. The objective's own gear argmin buys them only
by pushing the envelope 1.5 px OUTSIDE canonical's, and reaching them through
`Icon` needs a +7% `MARK_BOXES` height edit, the class rule 90 has refused three
times. Its dial was correctable and was corrected (a `circle` inside a non-uniform
scale draws an ellipse; ry 3 -> 3.43 took the band 8.8566 -> 8.6972), and that is
14.4% of the window; the cog is the other 86% and it is an asset difference.
Together `shield` 9.3281 and `gear` 8.6972 are 18.0 of a 4.36 screen's windows.

 Added after round 32; grade 18 refuted the standing claim
...
**5. CANONICAL'S TUNGSTEN IS A HEAVIER CUT THAN ANY BUNDLED ONE.** Added after
round 37; it is the measured cause of `digit0` 9.461 and `digit1` 10.470, the two
largest windows this screen had listed as unexplained. At a landed cap and a
landed advance, canonical's digit stems are pooled **13% wider** and its counters
**14% narrower** — the counters closing by twice the stem excess, which is what a
heavier CUT does and what a stroke does not. Rasterised with FreeType at the
shipped em the render is Tungsten **Medium to 0.03 px**, and canonical sits
between Medium and Semibold at fraction 0.39: an interpolated weight of about
**478**. Medium is 14% light, Semibold 19% heavy, and `font-weight` cannot express
478 because CSS matching sends 470 and 500 alike to the 400 face — so the
reachable set is {Medium, Semibold} and Medium is already the nearer of the two.
The `wordmark` is the control that this is not a global canonical darkening: Geist
stems there read C/R 0.9567, i.e. canonical is LIGHTER.

  * Option A: ship Medium and accept the weight residual, stated.
  * Option B: license the weight canonical actually used — the same purchase as
    decisions (1) and (2), and it would close the digits outright.
  * Option C: synthesise it with `-webkit-text-stroke-width`, which this file
    declares and never uses. A sweep puts the per-glyph argmin at 0.4-0.8 px for
    three digits and at ZERO for the "2", and it buys the stem at the price of a
    cap currently landed to 0.6%. That is the trade this file declines every time,
    and it is listed for completeness rather than recommended.

**These five interact, and that is the argument for taking them together.**
