...
- Focused live-capture session summary unit result bundle:
...
- Focused flaw-generation unit result bundle:
...
Image-fill policy: canonical screens with no functional customer media still
need purposeful basketball imagery or a meaningful generated/placeholder visual
that reflects the page's job. Blank decorative voids are not considered done,
even when the page has no upload/photo feature.
...
| 54 | Training Home | Yes | Quick start, my drills, calendar, drill detail, tracker routes asserted; focused proof now verifies latest weak analysis changes target/recommendation, empty-history placeholders replace fake stats, and a completed shot-tracker session persists back into the recent-workout card with `3` shots, `2` makes, `66.7%`, and form score `70` | Backend training-plan/workout reload, goal-derived recommendations, and web parity |
| 55 | Quick Start | Yes | Latest weak analysis changes target/score/verdict, workout history changes next shot/make targets and note, target steppers work, old `62.5%` demo stat is absent, and Start shot tracking opens the selected drill | Backend workout reload, live training-plan/goal mutations, physical-device behavior, and web parity |
...
| 57 | Drill Detail | Yes | Latest weak analysis changes selected drill plan to `STACK & SHOOT`, score `82`, `Beginner`, `8 min`, `24-30 reps`, analysis-derived build/cue/mechanics copy, save toast, My Drills metadata persistence, calendar/media side routes, and Start Drill payload; old fixed `Advanced` / `60-70 reps` values are absent | Backend drill-plan reload, live goal/flaw mutations, physical-device behavior, and web parity |
...
| 60 | Drill Execution | Yes | Latest weak analysis and selected `STACK & SHOOT` plan drive drill name, cue, focus, media key, and target makes; view-angle menu, make/miss/undo, pause/resume, toast feedback, live stats, target remaining, and completion route/totals proven | Backend shot-event/workout reload, live camera/pose-driven cues, physical-device media/progress behavior, and web parity |
| 61 | Shot Tracker | Yes | Recent workout card opens tracker; zero-state timer/media placeholder/progress rail/phase rail/score bar proven; empty End Workout shows a toast and stays on tracker; pause/resume, View Analysis, make/miss, undo, saving progress, derived percentage/streak, and completion totals proven | Backend shot-session reload, analytics aggregation, physical-device behavior, and web parity |
...
- `/Volumes/TBF SKILLZ.INC/CodexWork/shotiq-test-results/ShotIQ-LatePageRegressionIsolated-2026-08-08-v2.xcresult`
  ...
  `testGoalsImageSurfacesWork`, and `testGoalDetailImageSurfacesWork`.
  Result: `** TEST SUCCEEDED **`, 9 passed, 0 failed, 0 skipped. This proves
  the late analytics/profile/player-card/media/goals screens as isolated staged
  launches, with media play/frame toasts, media selection, linked-analysis
  navigation, goal-detail navigation, and customize-card save feedback.
- `/Volumes/TBF SKILLZ.INC/CodexWork/shotiq-test-results/ShotIQ-SelectedMediaLibrary-2026-08-08-v2.xcresult`
  ran
  `ShotIQUITests/ShotIQUITests/testSelectedPhotoAnalysisAppearsInMyMediaAndDetail`
  on the iPhone 17 Pro simulator. Result: `** TEST SUCCEEDED **`, 1 passed, 0
...
- The older bundled media/profile regression attempts
  (`ShotIQ-MediaProfileRegression-2026-08-08-v1.xcresult`,
  `ShotIQ-MediaProfileRegression-2026-08-08-v2.xcresult`,
  `ShotIQ-MediaProfileRegression-2026-08-08-v3.xcresult`, and
  `ShotIQ-LatePageRegressionSplit-2026-08-08-v1/v2.xcresult`) are superseded.
  Attachment hierarchies showed staged relaunches falling back to
  `screen-ios-splash`, so late-page staged proof now stays isolated to one
  staged cold launch per test method.
...
- `/Volumes/TBF SKILLZ.INC/CodexWork/shotiq-test-results/ShotIQ-FlawGeneration-Unit-2026-08-08-v3.xcresult`
  ...
  Result: `** TEST SUCCEEDED **`, 1 passed, 0 failed, 0 skipped. This
  proves saved weak metrics generate non-demo flaws.
- `/Volumes/TBF SKILLZ.INC/CodexWork/shotiq-test-results/ShotIQ-FlawsOverviewWeakAnalysis-2026-08-08-v1.xcresult`
  ran
